...
- 🔍 **Repository-aware chat** — the project chat is given real evidence (whole-repo file tree via one Git Trees API call, README, manifests), cached per account+repo+branch, with a branch self-heal and a **repository health banner** (`GET /projects/:id/repo-status`) that states *why* a repository could not be read instead of letting the model invent a 404.
