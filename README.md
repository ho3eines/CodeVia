...
- 📦 **Clone-first repository reads** — the chat and agents read each project from a local shallow clone kept fresh in `WORKSPACES_DIR` (git clone / tarball / API fallback), so the AI sees the real tree + README + source instantly and never fabricates a "404" when the Contents API is slow; writes still flow through branch → atomic commit → draft PR → CI → human merge.
