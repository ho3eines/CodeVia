...
export function repositoryForAgent(project: Project, type: AgentType): ProjectRepositoryLink {
  const roles =
    type === "frontend-developer" || type === "uiux"
      ? ["frontend", "mobile"]
      : type === "documentation"
        ? ["docs"]
        : ["backend"];
...
export function assertWriter(agent: Agent): void {
  ...
  if (
    !write ||
    !["read_file", "write_file", "create_branch", "create_pull_request"].every((t) => agent.tools.includes(t))
  ) {
    throw new Error(
      `${agent.name} needs read_file, write_file, create_branch and create_pull_request with repository write permission; refusing a base-branch fallback`,
    );
  }
}
...
export function defaultItem(
  ...
  return {
    ...
    files: [
      ...new Set(
        [owned ?? scaffold?.path, notePathFor(task.id, type.replace("-developer", ""), slugify(task.title))].filter(
          (p): p is string => !!p,
        ),
      ),
    ],
...
export function parseBreakdown(raw: string, allowed: AgentType[]): BreakdownItem[] {
  ...
  if (!Array.isArray(parsed) || parsed.length === 0 || parsed.length > MAX_SUBTASKS)
    throw new Error(`Expected 1–${MAX_SUBTASKS} implementer subtasks`);
  const items = parsed.map((value, index): BreakdownItem => {
    const item = value as Record<string, unknown> | null;
    if (!item || !allowed.includes(item.agentType as AgentType) || typeof item.title !== "string" || !item.title.trim())
      throw new Error("Breakdown selected an unavailable implementer or empty title");
    if (item.description !== undefined && (typeof item.description !== "string" || item.description.length > 6000))
      throw new Error("Subtask description must be text of at most 6000 characters");
    if (!Array.isArray(item.files) || item.files.length < 1 || item.files.length > 5)
      throw new Error("Each subtask requires 1–5 file paths");
    ...
    if (typeof id !== "string" || !/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(id))
      throw new Error("Subtask id must be a short stable identifier");
    const strings = (value: unknown, label: string, max: number): string[] | undefined => {
      if (value === undefined) return undefined;
      if (
        !Array.isArray(value) ||
        value.length > max ||
        value.some((s) => typeof s !== "string" || !s.trim() || s.length > 600)
      )
...
export function orderBreakdown(items: BreakdownItem[]): BreakdownItem[] {
  ...
  const rank: Partial<Record<AgentType, number>> = {
    database: 0,
    "backend-developer": 1,
  ...
  while (remaining.length) {
    const ready = remaining
      .filter((item) => (item.dependsOn ?? []).every((dep) => done.has(dep)))
...
export async function prepareImplementation(
  ...
  assertWriter(agent);
  if (!ctx.chat && ctx.github.kind === "real")
    throw new Error("No real coding model configured. Simulation scaffolds cannot be committed to a real repository.");
  const paths = item.files.map(cleanRepoPath);
  if (!paths.length || paths.length > 5 || new Set(paths).size !== paths.length)
    throw new Error("Expected 1–5 distinct file paths");
  ...
  for (const target of paths) {
    ...
    const pack = await buildContextPack({
      ...
      target,
      entityRoute: entityFor(item.title, item.description).route,
      strict: true,
    });
    ...
    if (existing !== undefined && existing.length > MAX_FILE_CHARS)
      throw new Error(`${target} is too large to edit safely; split the task instead of truncating it`);
    let content: string;
    if (ctx.chat) {
      ...
      content = extendContent({
        ...
        subtaskId: child.id,
        todos: [item.description || item.title, ...(opts.fixContext ? [opts.fixContext] : [])],
      });
    } else {
      const scaffold = scaffoldFor({
        agentType: agent.type,
        project: ctx.project,
        task: { ...child, title: item.title.replace(/^Implement \([^)]*\):\s*/i, ""), description: item.description },
        childId: child.id,
        brief: opts.brief,
        fixContext: opts.fixContext,
      });
      if (!scaffold || scaffold.path !== target)
        throw new Error(`Simulation cannot generate ${target}; configure a real coding model`);
  ...
  ctx.checkActive();
  ctx.setSummary(
    `${ctx.chat ? "Implementation" : "Simulation scaffold (not a completed feature)"}: ${item.title}\nRepository: ${ctx.project.configRepo}\nBranch: ${opts.branch}\nFiles:\n${paths.map((p) => `- ${p}`).join("\n")}`,
  );
  return [
    ...
    ...(working
      ...
      : [
          {
            label: `Create branch ${opts.branch}`,
            tool: "create_branch",
            input: { name: opts.branch, from: opts.baseBranch, sha: source.sha },
          },
        ]),
    {
...
export function pullRequestStep(
  agent: Agent,
  task: Task,
  repo: string,
  branch: string,
  ...
  return {
    ...
    input: {
      ...
      head: branch,
      base,
      title: `[CodeVia] ${task.title}`.slice(0, 120),
      taskTitle: task.title,
      taskDescription: task.description,
      changes: paths,
      tests: ["Verification pending. Do not merge until the task's GitHub CI gate passes."],
      risks: ["Generated change; human review and passing CI are required before merge."],
      draft: true,
    },
  };
}
...
export async function prepareSingleImplementation(
  ctx: PrepareContext,
  agent: Agent,
  task: Task,
  memoryRepo?: MemoryRepository,
  ...
  assertWriter(agent);
  const branch = task.workflowId
    ? `agent-task-${task.id.replace(/^task-/, "")}`
    : `agent-${agent.slug}-${task.id.replace(/^task-/, "")}`.slice(0, 80);
  const [owner, name] = ctx.project.configRepo.split("/");
  const exists = (await ctx.github.listBranches({ owner, name })).some((b) => b.name === branch);
  const pack = await buildContextPack({
    github: ctx.github,
    project: { ...ctx.project, branch: exists ? branch : ctx.project.branch },
    memoryRepo,
    strict: true,
  });
  ...
  if (ctx.chat) {
    const raw = await ctx.chat.chat(
      "Plan this single-agent implementation. Reply ONLY with a JSON array containing one subtask.",
      `Task: ${task.title}\n${task.description}\n${renderPromptContext(pack, "(planning)")}\nAllowed agentType: ${agent.type}. Schema: [{"agentType","title","description","files":[1–5 repository-relative file paths]}]. Reuse existing files.`,
      2000,
    );
  ...
  ctx.assignSkills({
    ...
    input: {
      ...
      skillInstructions: Object.keys(item.skillInstructions ?? {}).length
        ? item.skillInstructions
        : task.input.skillInstructions,
    },
  });
  const handoff: string[] = [];
  const workflow = task.input?.workflow as { artifacts?: unknown } | undefined;
  if (Array.isArray(workflow?.artifacts)) {
    for (const artifact of workflow.artifacts as Array<{
      repo: string;
      branch: string;
      sha?: string;
      files: string[];
      ...
      if (!ctx.project.repositories?.some((r) => r.repo === artifact.repo))
        throw new Error("Handoff repository is not linked to this project");
      ...
      for (const path of artifact.files.filter((p) => !p.startsWith("docs/tasks/"))) {
        ...
        handoff.push(
          `--- ${artifact.repo}@${artifact.sha ?? artifact.branch}:${path} (context excerpt) ---\n${file.content.slice(0, 12000)}`,
        );
      }
    }
  }
  const plan = await prepareImplementation(ctx, agent, task, item, {
    branch,
    baseBranch: ctx.project.branch,
  ...
  if (!agent.tools.includes("run_tests"))
    throw new Error(`${agent.name} needs run_tests to verify a standalone implementation`);
  plan.push({
    label: "Verify build and tests (GitHub CI)",
    tool: "run_tests",
    input: { repo: ctx.project.configRepo, ref: branch },
  });
  return plan;
}
