...
export class AgentRouter {
  ...
  route(problem: string, mode: "error" | "task" = "task"): AgentType {
    ...
    const map: Array<[RegExp, AgentType]> = [
      [
        /ui|ux|frontend|front-end|component|style|css|responsive|accessib|page|layout|mobile|رابط|ظاهر|فرانت|صفحه|کامپوننت|استایل|ریسپانسیو|موبایل/,
        "uiux",
      ],
      ...
      [
        /security|vulnerab|injection|csrf|xss|secret|oauth|authn|authz|امنیت|آسیب|توکن|سکرت|احراز|مجوز|نفوذ/,
        "security",
      ],
      ...
      [
        /architecture|boundaries|layering|ddd|bounded context|high-level design|معماری|ساختار|لایه|طراحی\s*سیستم/,
        "system-architect",
      ],
      [
        /api|controller|service|backend|back-end|handler|endpoint|login|auth|بک\s*اند|بکند|سرور|سرویس|کنترلر|ای\s*پی\s*آی|لاگین|ورود/,
        "backend-developer",
      ],
      ...
      [
        /research|feasib|best practice|compare|landscape|options|تحقیق|بررسی|مقایسه|بهترین\s*روش|امکان\s*سنجی/,
        "research",
      ],
