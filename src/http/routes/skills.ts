...
export function registerSkillRoutes(app: FastifyInstance, container: Container): void {
  const save = async (skill: Skill): Promise<Skill> => {
    ...
    if (s.projectId) {
      ...
      await container.projectFiles.writeFiles(
        p,
        [{ path: statePath(SKILL_DIR, s.slug), content: renderSkillFile(s), sourceSha: skill.repositoryRevision }],
        "[CodeVia] save skill definition",
      );
    }
  ...
  app.get("/skills", { schema: { tags: ["skills"] } }, async (req) => {
    ...
    if (q.q)
      skills = skills.filter((s) => `${s.name} ${s.description} ${s.slug}`.toLowerCase().includes(q.q!.toLowerCase()));
    return skills;
  });
  app.get("/skills/categories", { schema: { tags: ["skills"] } }, async () => [
    ...new Set(container.skillRepo.globalCatalog().map((s) => s.category)),
  ]);
  app.post("/skills", { schema: { tags: ["skills"] } }, async (req) => {
    ...
    if (container.skillRepo.findBySlug(slug, projectId))
      throw Object.assign(new Error(`Skill slug ${slug} already exists`), { statusCode: 409 });
    const now = new Date().toISOString();
    let draft = skillSchema.parse({
      id: projectId ? localId(projectId, "skill", slug) : randomUUID(),
      projectId,
      slug,
  ...
  app.patch("/skills/:id", { schema: { tags: ["skills"] } }, async (req, reply) => {
    ...
    if (old.projectId && b.slug !== undefined && b.slug !== old.slug)
      return reply.code(400).send({ error: "Rename the skill and its references in CodeVia together, then refresh" });
