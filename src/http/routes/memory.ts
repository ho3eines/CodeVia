...
export function registerMemoryRoutes(app: FastifyInstance, container: Container): void {
  app.get("/memory", { schema: { tags: ["memory"] } }, async (req) => {
    ...
    let data = q.projectId
      ? container.memoryRepo.byProject(q.projectId).filter(() => owned.has(q.projectId!))
      : container.memoryRepo
          .findMany()
          .map((r) => r.data)
          .filter((d) => !d.projectId || owned.has(d.projectId));
  ...
  app.post("/memory", { schema: { tags: ["memory"] } }, async (req, reply) => {
    ...
    const type = String(b.type ?? "knowledge"),
      key = String(b.key ?? "entry");
    const entry = memorySchema.parse({
      id: projectId ? localId(projectId, "memory", `${type}\0${key}`) : randomUUID(),
      projectId,
      type,
      key,
    ...
    if (projectId) {
      ...
      await container.projectFiles.updateMemory(p, (entries) => {
        if (entries.some((e) => e.id === entry.id))
          throw Object.assign(new Error("Memory key/type already exists; update it explicitly"), { statusCode: 409 });
  ...
  app.patch("/memory/:id", { schema: { tags: ["memory"] } }, async (req, reply) => {
    ...
    if (rec.projectId) {
      ...
      await container.projectFiles.updateMemory(p, (entries) => {
        ...
        if (entries.some((e) => e.id !== id && e.id === updated.id))
          throw Object.assign(new Error("Memory key/type already exists"), { statusCode: 409 });
        return [...entries.filter((e) => e.id !== id), updated];
      });
    } else {
      updated = edit(rec);
      container.memoryRepo.upsert(updated, { key: updated.key });
    }
    return updated!;
  });
