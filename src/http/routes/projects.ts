...
export function registerProjectRoutes(app: FastifyInstance, container: Container): void {
  ...
  // Pull the project's own folder (CodeVia/*) back into the database —
  // agents, tasks, memory, skills and definition are restored from git.
  app.post("/projects/:id/pull", { schema: { tags: ["projects"] } }, async (req, reply) => {
    ...
    container.auditRepo.record({
      action: "project.pulled",
      projectId: id,
      result: "success",
      source: "web",
      correlationId: `pull-${id}-${Date.now()}`,
      metadata: { agents: summary.agents, tasks: summary.tasks, memory: summary.memory },
    });
    return summary;
  });

  // Browse the project folder in git (paths + file content).
