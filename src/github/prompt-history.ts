...
const versionSchema = z.object({
  id: z.string().min(1), projectId: z.string().min(1), agentId: z.string().min(1),
  version: z.number().int().positive(), systemPrompt: z.string().max(100_000),
  projectPrompt: z.string().max(100_000).optional(), source: z.string(),
  note: z.string().optional(), derivedFrom: z.number().int().positive().optional(),
  createdAt: z.string().min(1),
});
const historySchema = z.object({
  projectId: z.string().min(1), agentId: z.string().min(1),
  versions: z.array(versionSchema).max(10_000),
}).superRefine((history, ctx) => {
  const ids = new Set<string>(), versions = new Set<number>();
  for (const v of history.versions) {
    if (v.projectId !== history.projectId || v.agentId !== history.agentId) ctx.addIssue({ code: "custom", message: "Prompt version belongs to a different history" });
    if (ids.has(v.id) || versions.has(v.version)) ctx.addIssue({ code: "custom", message: "Duplicate prompt version identity or number" });
    ids.add(v.id); versions.add(v.version);
  }
});
...
export function renderPromptHistory(history: PromptHistory): string {
  ...
  return matter({ schemaVersion: STATE_VERSION, promptHistory: { ...safe, versions } }, [
