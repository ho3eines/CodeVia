...
export const skillSchema = z.object({
  ...
  version: z.string(),
  tools: strings,
  dependencies: z.array(slugSchema).max(64),
  compatibleAgentTypes: strings,
  ...
  const visit = (v: unknown): unknown => {
    ...
    return Object.fromEntries(
      Object.entries(v)
        .filter(
          ([key]) =>
            !/^(?:api[-_]?keys?|api[-_]?token|bot[-_]?token|github[-_]?token|telegram[-_]?token|bearer[-_]?token|auth[-_]?token|id[-_]?token|access[-_]?token|refresh[-_]?token|token|password|secret|client[-_]?secret|api[-_]?secret|secret[-_]?access[-_]?key|private[-_]?key|signing[-_]?key|connection[-_]?string|authorization|cookie|credentials|encryptedToken|encryptedSecret|secretValueEnc|tokenValueEnc|apiKeyEnc|passwordHash|tokenHash|githubConnection|ownerId|repositoryRevision)$/i.test(
              key,
            ),
        )
...
export function parseSkillFile(content: string): Skill {
  ...
  return skillSchema.parse({
    ...data,
    instructions:
      data.schemaVersion !== 2 && typeof data.instructions === "string"
        ? data.instructions
        : body.replace(/^\r?\n/, ""),
  });
}
...
export function assertVersion(data: Record<string, unknown>): void {
  if (data.schemaVersion !== undefined && data.schemaVersion !== STATE_VERSION)
    throw new Error(`Unsupported CodeVia schema version ${String(data.schemaVersion)}`);
}
...
export const renderRulesFile = (rules: string[]) =>
  matter({ schemaVersion: STATE_VERSION, rules }, `# Project rules\n\n${rules.join("\n\n")}`);
...
const workflowSchema = z.object({
  ...
  nodes: z.array(
    z.object({
      id: z.string(),
      type: z.enum(["agent", "tool", "condition", "approval", "parallel", "trigger", "webhook", "telegram"]),
      name: z.string(),
...
export function renderWorkflowFile(workflow: Workflow): string {
  return matter(
    { schemaVersion: STATE_VERSION, workflow: repositorySafe(workflow) },
...
export function renderRunFile(run: Run): string {
  return matter(
    { schemaVersion: STATE_VERSION, run: repositorySafe(run) },
    `# Run ${run.id}\n\n${run.summary ?? run.error ?? run.status}`,
  );
}
export function renderConversationFile(conversation: Conversation): string {
  return matter(
    { schemaVersion: STATE_VERSION, conversation: repositorySafe(conversation) },
...
export function parseHistoryFile<T extends Run | Conversation>(content: string, kind: "run" | "conversation"): T {
  ...
  assertVersion(data);
  const item = z
    .object({ id: z.string().min(1), projectId: z.string(), createdAt: z.string(), updatedAt: z.string() })
    .passthrough()
    .parse(data[kind]);
  if (kind === "run") {
    z.object({
      taskId: z.string(),
      agentId: z.string(),
      status: z.enum(["queued", "running", "succeeded", "failed", "cancelled", "waiting_for_approval"]),
      steps: z.array(z.object({ index: z.number(), label: z.string(), status: z.string() }).passthrough()),
    })
      .passthrough()
      .parse(item);
  } else {
...
export function parseMemoryState(content: string): MemoryEntry[] | undefined {
  const { data } = parseMatter(content);
  assertVersion(data);
  if (data.schemaVersion === 2 && data.entries === undefined)
    throw new Error("Version 2 memory requires an entries array");
...
export function assertNoCredentialMaterial(content: string): void {
  if (
    /\b(?:gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{25,}|sk-(?:proj-|ant-)?[A-Za-z0-9_-]{24,})\b|-----BEGIN (?:[A-Z]+ )?PRIVATE KEY-----|(?:https?|postgres(?:ql)?|mysql|mongodb(?:\+srv)?|redis):\/\/[^\s/:]{1,256}:[^\s/@]{1,4096}@/.test(
      content,
    )
  ) {
