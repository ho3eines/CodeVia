...
import {
  ...
  parsePromptHistory,
  parsePromptHistoryIndex,
  renderPromptHistory,
  renderPromptHistoryIndex,
  type PromptHistory,
...
const definitionSchema = z
  .object({
    ...
    capabilities: z.object({
      ...
      frameworks: stringList.default([]),
      databases: stringList.default([]),
      deploymentTargets: stringList.default([]),
      features: stringList.default([]),
      integrations: stringList.default([]),
      agentTypes: stringList.default([]),
    }),
    repositories: z.array(z.object({ repo: z.string(), branch: z.string(), role: z.string() }).passthrough()).min(1),
    ...
    repositoryState: z
      .object({
        version: z.literal(2),
        generation: z.enum(["ai", "simulation", "imported"]),
    ...
    settings: z
      .object({
        ...
        notifications: stringList,
        rules: stringList,
        skills: stringList,
        generatedSkills: stringList.optional(),
        workflows: stringList,
        budget: z.object({
          maxTokensPerRun: z.number().finite().nonnegative(),
          maxCallsPerRun: z.number().finite().nonnegative(),
          maxCostUsdPerRun: z.number().finite().nonnegative(),
...
const stateError = (message: string, cause?: unknown) =>
  Object.assign(new Error(`CodeVia repository state: ${message}${cause ? ` (${String(cause)})` : ""}`), {
    statusCode: 502,
    retryable: false,
  });
const conflict = (path: string) =>
  Object.assign(
    new Error(
      `CodeVia conflict at ${path}; the repository changed since it was read. Refresh and retry; no files were overwritten.`,
    ),
...
export class ProjectFilesService {
  ...
  private key(p: Project): string {
    return `${p.configRepo}@${p.branch}`;
  }
  private cacheKey(p: Project): string {
    return `${p.id}:${this.key(p)}`;
  }
  private github(p: Project): IGitHubService {
    return this.deps.githubForProject?.(p) ?? this.deps.github;
  }
  private ref(p: Project): GithubRepoRef {
    const [owner, name] = p.configRepo.split("/");
    if (!owner || !name) throw stateError("a connected repository is required");
    return { owner, name };
  }
  private async locked<T>(p: Project, fn: () => Promise<T>): Promise<T> {
    const key = this.key(p);
    const prev = this.locks.get(key) ?? Promise.resolve();
    const pending = prev.catch(() => undefined).then(fn);
  ...
  async removeProject(p: Project): Promise<void> {
    ...
    try {
      ...
      let tree;
      try {
        tree = await gh.listFiles(this.ref(p), sha, CODEVIA_DIR);
      } catch (error) {
  ...
  private async seedMissingMockRepos(p: Project): Promise<boolean> {
    ...
    const mock = gh as unknown as {
      seedRepo(
        owner: string,
        name: string,
        opts?: { files?: GithubFile[]; branch?: string; description?: string },
      ): unknown;
    };
    ...
    for (const link of links) {
      ...
      mock.seedRepo(owner, name, {
        files: [{ path: "README.md", content: `# ${p.name}\n\n${p.description}\n` }],
        branch: link.branch,
  ...
  private async raw(p: Project): Promise<{ sha: string; contents: Map<string, string> }> {
    ...
    let tree;
    try {
      tree = await gh.listFiles(ref, sha, CODEVIA_DIR);
    } catch (error) {
      if ((error as { status?: number }).status !== 404) throw error;
    ...
    const paths = tree.filter((e) => e.type === "blob" && e.path.startsWith(`${CODEVIA_DIR}/`)).map((e) => e.path);
    if (new Set(paths).size !== paths.length || paths.some((path) => !isStatePath(path)))
      throw stateError("invalid/duplicate file path");
    ...
    for (let offset = 0; offset < paths.length; offset += 8) {
      const batch = await Promise.all(
        paths.slice(offset, offset + 8).map(async (path) => {
          ...
          if (Buffer.byteLength(file.content) > 8_000_000)
            throw stateError(`file ${path} exceeds the state size limit`);
          return [path, file.content] as const;
        }),
      );
      for (const [path, content] of batch) contents.set(path, content);
    }
    if ([...contents.values()].reduce((size, text) => size + Buffer.byteLength(text), 0) > 32_000_000)
      throw stateError("total project state exceeds the 32 MB limit");
  ...
  private decode(raw: { sha: string; contents: Map<string, string> }): RepositorySnapshot {
    const out: RepositorySnapshot = {
      ...raw,
      files: [...raw.contents.keys()],
      agents: [],
      tasks: [],
      memory: [],
      skills: [],
    ...
    for (const [path, content] of raw.contents) {
      const managed =
        [PROJECT_FILE, SKILLS_FILE, MEMORY_FILE, RULES_FILE, PROMPT_HISTORY_INDEX].includes(path) ||
        [SKILL_DIR, AGENTS_DIR, WORKFLOW_DIR, TASKS_DIR, RUN_DIR, CONVERSATION_DIR, PROMPT_HISTORY_DIR].some(
          (dir) => path.startsWith(`${dir}/`) && path.endsWith(".md"),
        );
      ...
      if (
        ...
        throw stateError("core files cannot be tombstoned; save an empty list instead");
      ...
      if (path === PROJECT_FILE) {
        ...
        parsePromptHistoryIndex(content);
        out.promptHistoryInitialized = true;
        ...
        assertVersion(data);
        out.skills = z.array(slugSchema).parse(data.skills);
        ...
        z.object({
          ...
          permissions: stringList.optional(),
        ...
        z.object({
          ...
          enabled: z.boolean(),
          maxIterations: z.number().finite().nonnegative(),
          timeoutMs: z.number().finite().nonnegative(),
          tokenBudget: z.number().finite().nonnegative(),
        }).parse(a);
        a.configPath = path;
        out.agents.push(a);
      } else if (path.startsWith(`${WORKFLOW_DIR}/`) && path.endsWith(".md"))
        out.workflows.push(parseWorkflowFile(content));
      else if (path.startsWith(`${TASKS_DIR}/`) && path.endsWith(".md")) {
        assertVersion(data);
        const t = parseTaskFile(content);
        if (!t) throw stateError(`invalid task ${path}`);
        z.enum(["created", "queued", "running", "succeeded", "failed", "cancelled", "waiting_for_approval"]).parse(
          t.status,
        );
        out.tasks.push(t);
      } else if (path.startsWith(`${RUN_DIR}/`) && path.endsWith(".md"))
        out.runs.push(parseHistoryFile<Run>(content, "run"));
      else if (path.startsWith(`${CONVERSATION_DIR}/`) && path.endsWith(".md"))
        out.conversations.push(parseHistoryFile<Conversation>(content, "conversation"));
    }
    for (const [name, values] of [
      ...
      ["task", out.tasks],
      ["run", out.runs],
      ["conversation", out.conversations],
    ] as const) {
      if (new Set(values.map((v) => v.id)).size !== values.length) throw stateError(`duplicate ${name} identity`);
    }
    if (new Set(out.promptHistories.map((h) => h.agentId)).size !== out.promptHistories.length)
      throw stateError("duplicate prompt history for an agent");
    ...
    if (new Set(out.skillDefinitions.map((s) => s.slug)).size !== out.skillDefinitions.length)
      throw stateError("duplicate skill slug");
    ...
    if (new Set(out.memory.map((m) => `${m.type}\0${m.key}`)).size !== out.memory.length)
      throw stateError("duplicate memory key/type");
    return out;
  }
  ...
  async pull(project: Project): Promise<RepositorySnapshot> {
    return this.locked(project, async () => {
      try {
        ...
        if (cached?.sha !== raw.sha)
          this.decoded.set(this.cacheKey(project), { sha: raw.sha, state: structuredClone(out) });
        this.baselines.set(this.cacheKey(project), new Map(raw.contents));
        return out;
      } catch (err) {
        throw stateError("read/validation failed; cached definitions were not substituted", err);
      }
    });
  }
  ...
  private async commit(
    p: Project,
    current: { sha: string; contents: Map<string, string> },
    files: GithubFile[],
    message: string,
  ): Promise<void> {
    ...
    for (const file of changed) {
      ...
      if (Buffer.byteLength(file.content) > 8_000_000)
        throw stateError(`file ${file.path} exceeds the state size limit`);
      staged.set(file.path, file.content);
    }
    if ([...staged.values()].reduce((size, text) => size + Buffer.byteLength(text), 0) > 32_000_000)
      throw stateError("total project state exceeds the 32 MB limit");
    let decoded: RepositorySnapshot;
    try {
      decoded = this.decode({ sha: current.sha, contents: staged });
    } catch (error) {
      throw Object.assign(new Error(`Invalid CodeVia definition; no files were written (${String(error)})`), {
        statusCode: 422,
        retryable: false,
      });
    }
    try {
      ...
      for (const f of changed) contents.set(f.path, f.content);
      this.decoded.set(this.cacheKey(p), {
        sha: committed.sha,
        state: { ...decoded, sha: committed.sha, contents: new Map(contents) },
      });
      ...
      if ((error as { status?: number })?.status === 404) {
        ...
        try {
          ...
          /* diagnostic only; never mask the original 404 */
        }
        throw stateError(
  ...
  async writeFiles(p: Project, files: StateWrite[], message: string, checkConflicts = true): Promise<boolean> {
    ...
    const result = await this.locked(p, async () => {
      ...
      await this.commit(p, current, files, message);
      return true;
    });
  ...
  pathFor(p: Project, kind: "workflow" | "conversation" | "task", id: string): string {
    return this.entityPath(
      p,
      kind,
      { workflow: WORKFLOW_DIR, conversation: CONVERSATION_DIR, task: TASKS_DIR }[kind],
      id,
    );
  }
  ...
  private promptPath(p: Project, agentId: string, agent?: Agent): string {
    ...
    if (current?.projectId === p.id) {
      ...
      const occupied = [...(this.entityPaths.get(this.cacheKey(p)) ?? [])].some(
        ([key, path]) => key.startsWith("prompt-history:") && key !== `prompt-history:${agentId}` && path === base,
      );
      ...
      return occupied
        ? `${base.slice(0, -3)}--${localId("CodeVia", "history-path", `${base}\0${current.createdAt}`).slice(-16)}.md`
        : base;
    }
  ...
  missingPromptHistoryFiles(
    p: Project,
    agents: Agent[],
    snapshot: RepositorySnapshot,
    records = this.deps.repositories?.promptVersionRepo?.byProject(p.id) ?? [],
  ): GithubFile[] {
    const repo = this.deps.repositories?.promptVersionRepo;
    if (!repo) return [];
    const files: GithubFile[] = [];
    if (!snapshot.promptHistoryInitialized)
      files.push({ path: PROMPT_HISTORY_INDEX, content: renderPromptHistoryIndex() });
    const byId = new Map(agents.map((a) => [a.id, a]));
    const ids = new Set([...byId.keys(), ...records.map((v) => v.agentId)]);
    for (const agentId of ids) {
      const agent = byId.get(agentId),
        path = this.promptPath(p, agentId, agent);
      ...
      if (history.length)
        files.push({ path, content: renderPromptHistory({ projectId: p.id, agentId, versions: history }) });
    }
    return files;
  }
  ...
  private promptWrites(p: Project, agents: Agent[], supplied?: PromptVersion[], all = false): StateWrite[] {
    ...
    if (supplied?.some((v) => v.projectId !== p.id || (!all && !ids.has(v.agentId))))
      throw stateError("prompt history write crosses project/agent boundaries");
    ...
    if (snapshot)
      for (const f of this.missingPromptHistoryFiles(p, [...knownAgents.values()], snapshot, records))
        files.set(f.path, f);
    for (const agentId of new Set(records.map((v) => v.agentId))) {
      ...
      files.set(path, {
        path,
        content: renderPromptHistory({ projectId: p.id, agentId, versions }),
        sourceSha: versions.find((v) => v.repositoryRevision)?.repositoryRevision ?? p.repositoryRevision,
      });
    }
    return [...files.values()];
  }
  ...
  async syncAll(
    ...
    input: {
      ...
      tasks: Task[];
      memory: MemoryEntry[];
      skillCatalog?: Array<{ slug: string; description?: string }>;
      workflows?: Workflow[];
      runs?: Run[];
      conversations?: Conversation[];
    ...
    const files: StateWrite[] = [
      ...
      ...input.agents.map((a) => ({
        path: this.agentPath(a),
        content: renderAgentFile(a),
        sourceSha: a.repositoryRevision,
      })),
      { path: SKILLS_FILE, content: renderSkillsFile(project, input.skillCatalog ?? []) },
      ...definitions.map((s) => ({
        path: statePath(SKILL_DIR, s.slug),
        content: renderSkillFile(s),
        sourceSha: s.repositoryRevision,
      })),
      ...input.tasks.map((t) => ({
        path: this.entityPath(project, "task", TASKS_DIR, t.id),
        content: renderTaskFile(t),
        runtime: true,
      })),
      { path: MEMORY_FILE, content: renderMemoryFile(input.memory) },
      ...(input.workflows ?? []).map((w) => ({
        path: this.entityPath(project, "workflow", WORKFLOW_DIR, w.id),
        content: renderWorkflowFile(w),
        sourceSha: w.repositoryRevision,
      })),
      ...(input.runs ?? []).map((r) => ({
        path: this.entityPath(project, "run", RUN_DIR, r.id),
        content: renderRunFile(r),
        runtime: true,
      })),
      ...(input.conversations ?? []).map((c) => ({
        path: this.entityPath(project, "conversation", CONVERSATION_DIR, c.id),
        content: renderConversationFile(c),
        sourceSha: c.repositoryRevision,
      })),
  ...
  agentPath(a: Agent): string {
    ...
    if (!isStatePath(path) || !path.startsWith(`${AGENTS_DIR}/`) || !path.endsWith(".md")) {
      ...
      throw stateError(
        `agent definition path must be inside CodeVia/agents/; agent ${a.id} (type "${a.type}", slug "${a.slug ?? a.type}") has configPath "${raw}". Move its definition under CodeVia/agents/ (or delete the stale record) and pull/refresh.`,
      );
    }
    return path;
  }
  syncTask(p: Project, t: Task): Promise<boolean> {
    return this.writeFiles(
      p,
      [{ path: this.entityPath(p, "task", TASKS_DIR, t.id), content: renderTaskFile(t), runtime: true }],
      `[CodeVia] task ${t.id} → ${t.status}`,
      false,
    );
  }
  syncRun(p: Project, r: Run): Promise<boolean> {
    return this.writeFiles(
      p,
      [{ path: this.entityPath(p, "run", RUN_DIR, r.id), content: renderRunFile(r), runtime: true }],
      `[CodeVia] run ${r.id} → ${r.status}`,
      false,
    );
  }
  syncAgents(p: Project, agents: Agent[], versions?: PromptVersion[]): Promise<boolean> {
    return this.writeFiles(
      p,
      [
        ...agents.map((a) => ({
        ...
        ...this.promptWrites(p, agents, versions),
      ],
      "[CodeVia] save agents and prompt history",
    );
  }
  syncSkills(p: Project, catalog: Array<{ slug: string; description?: string }> = []): Promise<boolean> {
    return this.writeFiles(
      p,
      [
        { path: SKILLS_FILE, content: renderSkillsFile(p, catalog) },
        ...catalog
          .filter((s): s is Skill => "instructions" in s)
          .map((s) => ({
            path: statePath(SKILL_DIR, s.slug),
            content: renderSkillFile(s),
            sourceSha: s.repositoryRevision,
  ...
  syncMemory(p: Project, entries: MemoryEntry[]): Promise<boolean> {
    return this.writeFiles(p, [{ path: MEMORY_FILE, content: renderMemoryFile(entries) }], "[CodeVia] save memory");
  }
  syncWorkflow(p: Project, w: Workflow): Promise<boolean> {
    return this.writeFiles(
      p,
      [
        {
          path: this.entityPath(p, "workflow", WORKFLOW_DIR, w.id),
          content: renderWorkflowFile(w),
          sourceSha: w.repositoryRevision,
        },
      ],
      "[CodeVia] save workflow",
    );
  }
  syncConversation(p: Project, c: Conversation): Promise<boolean> {
    return this.writeFiles(
      p,
      [
        {
          path: this.entityPath(p, "conversation", CONVERSATION_DIR, c.id),
          content: renderConversationFile(c),
          sourceSha: c.repositoryRevision,
        },
      ],
      "[CodeVia] save conversation",
    );
  }
  syncContext(p: Project, content: string): Promise<boolean> {
    return this.writeFiles(p, [{ path: RUNTIME_CONTEXT_FILE, content }], "[CodeVia] save runtime context", false);
  }
  async readContext(p: Project): Promise<string | undefined> {
    const state = await this.pull(p);
    return state.contents.get(RUNTIME_CONTEXT_FILE) ?? state.contents.get(CONTEXT_FILE);
  }
  /**
  ...
  tombstone(
    p: Project,
    path: string,
    meta: { kind?: string; id?: string; slug?: string; type?: string } = {},
  ): Promise<boolean> {
    return this.writeFiles(
      p,
      [
        {
          path,
          content: matter(
            { schemaVersion: 2, deleted: true, ...meta },
            "Intentionally removed. Do not regenerate automatically.",
          ),
        },
      ],
      "[CodeVia] remove definition",
    );
  }
  ...
  async updateMemory(p: Project, mutate: (entries: MemoryEntry[]) => MemoryEntry[]): Promise<MemoryEntry[]> {
    return this.locked(p, async () => {
      ...
      if (
        old?.trim() &&
        parseMatter(old).data.schemaVersion !== 2 &&
        !entries.some((e) => e.key === "_legacy/CodeVia-memory.md")
      ) {
        entries.push({
          id: localId(p.id, "memory", "knowledge\0_legacy/CodeVia-memory.md"),
          projectId: p.id,
          scope: "project",
          type: "knowledge",
          key: "_legacy/CodeVia-memory.md",
          content: old,
          tags: ["legacy-import"],
          refs: [],
  ...
  private memoryEntry(p: Project, e: ParsedMemoryEntry): MemoryEntry {
    return {
      ...e,
      id: localId(p.id, "memory", `${e.type}\0${e.key}`),
      projectId: p.id,
      scope: e.scope ?? "project",
      type: e.type as MemoryEntry["type"],
      refs: e.refs ?? [],
      source: e.source || "repository",
      createdAt: e.createdAt || e.updatedAt || p.createdAt,
      updatedAt: e.updatedAt || p.updatedAt,
    };
  }
  ...
  async restore(
    p: Project,
    repositories?: StateRepositories,
    opts: { includeTasks?: boolean; snapshot?: RepositorySnapshot } = {},
  ): Promise<PullSummary> {
    ...
    for (const [kind, entities] of [
      ...
      for (const entity of entities) {
        ...
        remap.set(`${kind}:${entity.id}`, mapped);
        local.add(`${kind}:${mapped}`);
      }
    }
    for (const h of snapshot.promptHistories) {
      const mapped = imported && h.projectId !== p.id ? localId(p.id, "agent", h.agentId) : h.agentId;
      remap.set(`agent:${h.agentId}`, mapped);
      local.add(`agent:${mapped}`);
      for (const v of h.versions) {
        const key = imported && v.projectId !== p.id ? localId(p.id, "prompt-version", v.id) : v.id;
        remap.set(`prompt-version:${v.id}`, key);
        local.add(`prompt-version:${key}`);
      }
    }
    const id = (kind: string, source: string | undefined): string | undefined =>
      source &&
      (remap.get(`${kind}:${source}`) ??
        (local.has(`${kind}:${source}`) || !imported ? source : localId(p.id, kind, source)));
    for (const e of snapshot.memory)
      if (e.id) remap.set(`memory:${e.id}`, localId(p.id, "memory", `${e.type}\0${e.key}`));
    const rebind = (value: unknown, key = ""): unknown => {
      if (Array.isArray(value)) return value.map((v) => rebind(v, key));
      if (value && typeof value === "object")
        return Object.fromEntries(Object.entries(value).map(([k, v]) => [k, rebind(v, k)]));
      if (typeof value !== "string") return value;
      const kind = /(?:agent|task|workflow|run|conversation|memory)Ids?$/i
        .exec(key)?.[0]
        .replace(/Ids?$/i, "")
        .toLowerCase();
    ...
    for (const [path, content] of snapshot.contents) {
      if (
        ![TASKS_DIR, WORKFLOW_DIR, RUN_DIR, CONVERSATION_DIR, PROMPT_HISTORY_DIR].some(
          (dir) => path.startsWith(`${dir}/`) && path.endsWith(".md"),
        ) ||
      ...
      for (const [kind, dir, key] of [
        ["task", TASKS_DIR, undefined],
        ["workflow", WORKFLOW_DIR, "workflow"],
        ["run", RUN_DIR, "run"],
        ["conversation", CONVERSATION_DIR, "conversation"],
      ] as const) {
    ...
    const apply = () => {
      ...
      if (snapshot.manifest?.definition) {
        ...
        if (!links.some((r) => r.repo === p.configRepo && r.branch === p.branch)) {
          if (!imported)
            throw stateError("manifest cannot redirect the connected repository; update the connection explicitly");
        ...
        project = hydrateProject({
          ...
          id: p.id,
          slug: stored.slug,
          repositories: links.map((r) => ({ ...r, isConfigRepo: r.repo === p.configRepo && r.branch === p.branch })),
          configRepo: p.configRepo,
          branch: p.branch,
          defaultModelId: def.defaultModelId ?? undefined,
          defaultAgentId: id("agent", def.defaultAgentId ?? undefined),
          telegramChatId: def.telegramChatId ?? undefined,
          settings: {
            ...def.settings,
            permissions: def.settings.permissions as Project["settings"]["permissions"],
            workflows: def.settings.workflows.map((w) => id("workflow", w)!),
          },
          repositoryState: def.repositoryState ?? { version: 2, generation: "imported", initializedAt: now },
        } as Project);
      } else if (snapshot.manifest) {
        const m = snapshot.manifest,
          prompt = m.promptSettings as Partial<Project["settings"]> | undefined;
        project = hydrateProject({
          ...stored,
          description: typeof m.description === "string" ? m.description : stored.description,
          capabilities: (m.capabilities as Project["capabilities"]) ?? stored.capabilities,
          settings: {
            ...stored.settings,
            ...(prompt?.rules ? { rules: stringList.parse(prompt.rules) } : {}),
            ...(prompt?.environment
              ? { environment: z.enum(["development", "staging", "production"]).parse(prompt.environment) }
              : {}),
            ...(prompt?.generatedSkills ? { generatedSkills: stringList.parse(prompt.generatedSkills) } : {}),
          },
        });
      }
      if (snapshot.skillsLoaded) project = { ...project, settings: { ...project.settings, skills: snapshot.skills } };
      if (snapshot.rules !== undefined)
        project = { ...project, settings: { ...project.settings, rules: snapshot.rules } };
      ...
      if (project.repositoryState || snapshot.manifest || snapshot.agents.length) {
        ...
        for (const cached of repos.agentRepo.byProject(p.id))
          if (!keep.has(cached.id)) repos.agentRepo.deleteById(cached.id);
      }
      for (const a of snapshot.agents) {
        ...
        const agent: Agent = {
          ...a,
          repositoryRevision: snapshot.sha,
          id: key,
          projectId: p.id,
          ...
          createdAt: a.createdAt ?? existing?.createdAt ?? now,
      ...
      if (repos.promptVersionRepo) {
        ...
        for (const h of snapshot.promptHistories) {
          ...
          if (!snapshot.promptHistoryInitialized)
            for (const v of repos.promptVersionRepo.forAgent(agentId))
              if (v.projectId === p.id) repos.promptVersionRepo.deleteById(v.id);
          for (const v of h.versions) {
            const key = id("prompt-version", v.id)!;
            const existing = repos.promptVersionRepo.findById(key)?.data;
            if (existing && existing.projectId !== p.id)
              throw stateError(`prompt version identity crosses projects: ${key}`);
            repos.promptVersionRepo.upsert(
              { ...v, id: key, agentId, projectId: p.id, repositoryRevision: snapshot.sha },
              { projectId: p.id, parentId: agentId },
            );
          }
        }
      }
      if (repos.skillRepo && (project.repositoryState || snapshot.skillDefinitions.length)) {
        ...
        for (const s of snapshot.skillDefinitions)
          repos.skillRepo.upsert(
            { ...s, repositoryRevision: snapshot.sha, projectId: p.id, id: localId(p.id, "skill", s.slug) },
            { projectId: p.id, key: s.slug },
          );
      }
      if (project.repositoryState || snapshot.contents.has(MEMORY_FILE))
        this.indexMemory(
          p,
          snapshot.memory.map((e) => this.memoryEntry(p, { ...e, refs: rebind(e.refs ?? [], "refs") as string[] })),
          repos.memoryRepo,
        );
          ...
          repos.workflowRepo.upsert(
            {
              ...
              nodes: w.nodes.map((n) => ({
                ...
                config: {
                  ...n.config,
                  ...(typeof n.config.agentId === "string" ? { agentId: id("agent", n.config.agentId) } : {}),
                },
              })),
            },
            { projectId: p.id },
          ...
          if (existing) {
            ...
            if (!["created", "queued", "running", "waiting_for_approval"].includes(existing.status)) {
              ...
              const changed =
                ...
                existing.error !== (t.error ?? undefined) ||
                (brief !== undefined && existing.input?.researchBrief !== brief);
              if (changed)
          ...
          repos.taskRepo.upsert(
            {
              ...
              assignedAgentId: id("agent", t.assignedAgentId),
              workflowId: id("workflow", t.workflowId),
              parentTaskId: id("task", t.parentTaskId),
              status: interrupted ? "failed" : t.status,
              error: interrupted ? "Restored interrupted execution; review before explicitly retrying." : t.error,
              correlationId: t.correlationId ?? `restored-${key}`,
              result: rebind(t.result) as Task["result"],
              input: {
                ...(rebind(t.input) as Task["input"]),
                ...(t.researchBrief ? { researchBrief: t.researchBrief } : {}),
                ...(Array.isArray(t.input?.dependsOn)
                  ? { dependsOn: t.input.dependsOn.map((dep) => id("task", String(dep))) }
                  : {}),
              },
            },
            { projectId: p.id, parentId: id("task", t.parentTaskId) },
      ...
      if (opts.includeTasks !== false && repos.runRepo)
        for (const r of snapshot.runs) {
          const key = id("run", r.id)!;
          const existing = repos.runRepo.findById(key)?.data;
          if (existing && existing.projectId !== p.id) throw stateError(`run identity crosses projects: ${key}`);
          if (!existing)
            repos.runRepo.upsert(
              {
                ...r,
                steps: rebind(r.steps) as Run["steps"],
                id: key,
                projectId: p.id,
                taskId: id("task", r.taskId)!,
                agentId: id("agent", r.agentId)!,
                workflowId: id("workflow", r.workflowId),
                status: ["running", "queued", "waiting_for_approval"].includes(r.status) ? "failed" : r.status,
              },
              { projectId: p.id, parentId: id("task", r.taskId) },
            );
          ...
          // stay runtime-owned.
            ...
            const changed =
              existing.summary !== r.summary || existing.verification !== r.verification || existing.error !== r.error;
            if (changed)
              repos.runRepo.upsert(
                { ...existing, summary: r.summary, verification: r.verification, error: r.error },
                { projectId: p.id, parentId: id("task", r.taskId) },
              );
          }
        }
      ...
      if (repos.conversationRepo && project.repositoryState && snapshot.conversations.length > 0) {
        ...
        for (const r of repos.conversationRepo.findMany({ projectId: p.id }))
          if (!keep.has(r.data.id)) repos.conversationRepo.deleteById(r.data.id);
      }
      if (repos.conversationRepo)
        for (const c of snapshot.conversations) {
          const key = id("conversation", c.id)!;
          const existing = repos.conversationRepo.findById(key)?.data;
          if (existing && existing.projectId && existing.projectId !== p.id)
            throw stateError(`conversation identity crosses projects: ${key}`);
          repos.conversationRepo.upsert(
            {
              ...c,
              repositoryRevision: snapshot.sha,
              id: key,
              projectId: p.id,
              activeAgentId: id("agent", c.activeAgentId),
            },
            { projectId: p.id, parentId: c.userId },
          );
        }
    };
    if (this.deps.transaction) this.deps.transaction(apply);
    else apply();
    this.entityPaths.set(this.cacheKey(p), paths);
    return {
      promptVersions: snapshot.promptHistories.reduce((count, h) => count + h.versions.length, 0),
      agents: snapshot.agents.length,
      tasks: opts.includeTasks === false ? 0 : snapshot.tasks.length,
