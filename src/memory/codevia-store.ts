...
export class CodeViaMemoryStore implements IMemoryStore {
  ...
  private async entries(): Promise<MemoryEntry[]> {
    ...
    if (body === undefined)
      throw new Error("CodeVia/memory.md is missing; initialize project state before reading memory");
    return parseMemoryFile(body).map((e) => ({
      ...e,
      id: localId(this.project.id, "memory", `${e.type}\0${e.key}`),
      ...
      refs: e.refs ?? [],
      source: e.source,
      createdAt: e.createdAt || e.updatedAt || this.project.createdAt,
      updatedAt: e.updatedAt || this.project.updatedAt,
    }));
  }
  async get(type: MemoryType, key: string): Promise<string | undefined> {
    return (await this.entries()).find((e) => e.type === type && e.key === key)?.content;
  }
  ...
  private async write(record: MemoryRecord, append: boolean): Promise<string> {
    await this.files.updateMemory(this.project, (entries) => {
      ...
      const entry: MemoryEntry = {
        ...record,
        id: localId(this.project.id, "memory", `${record.type}\0${record.key}`),
        projectId: this.project.id,
