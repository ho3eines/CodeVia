...
export function validateWorkflowGraph(workflow: Workflow): void {
  if (!Array.isArray(workflow.nodes) || !workflow.nodes.length || workflow.nodes.length > 256)
    throw new Error("Workflow needs 1–256 nodes");
  if (!Array.isArray(workflow.edges) || workflow.edges.length > 2048)
    throw new Error("Workflow may have at most 2048 edges");
  ...
  for (const node of workflow.nodes) {
    if (
      ...
      throw new Error(`Invalid or duplicate workflow node id: ${node?.id}`);
    ...
    if (!node.config || typeof node.config !== "object" || Array.isArray(node.config))
      throw new Error(`Invalid config for workflow node ${node.id}`);
    ids.add(node.id);
  }
  ...
  for (const edge of workflow.edges) {
    ...
    if (edge.condition !== undefined && typeof edge.condition !== "string")
      throw new Error("Invalid workflow edge condition");
    edges.add(key);
