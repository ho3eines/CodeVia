...
const config: ModelProvider = {
  id: "direct", name: "Ptero", type: "custom-http", apiFormat: "custom",
  baseUrl: "https://ptero.pro/wp-json/mlp/v1/chat", authType: "bearer", secretRef: "MLP_API_KEY",
  timeoutMs: 1000, defaultTemperature: 0.3, maxTokensDefault: 4096,
  rateLimitPerMinute: 10, active: true, createdAt: "", updatedAt: "",
};
const messages = [{ role: "user" as const, content: "Hello" }];
afterEach(() => { vi.unstubAllGlobals(); vi.unstubAllEnvs(); });
function setup(payload: unknown = { text: "Hello back" }, status = 200) {
  vi.stubEnv("MLP_API_KEY", "test-only-key");
  const f = vi.fn(async () => new Response(JSON.stringify(payload), { status, headers: { "content-type": "application/json" } }));
...
describe("direct JSON chat", () => {
  ...
  it("rejects malformed responses and native tool requests", async () => {
    ...
    f.mockClear();
    await expect(provider.chat({ modelId: "codestral:free", messages, tools: [{ name: "run", parameters: {} }] })).rejects.toThrow("tool calling");
    expect(f).not.toHaveBeenCalled();
  });
  it("reports HTTP failures and preserves OpenAI URLs", async () => {
    setup({ message: "No route" }, 404);
    expect(await testModelChat(config, "codestral:free")).toMatchObject({ ok: false, status: 404, url: config.baseUrl });
    expect(buildChatEndpoint({ ...config, apiFormat: "openai", baseUrl: "https://example.com/v1" })).toBe("https://example.com/v1/chat/completions");
  });
});
