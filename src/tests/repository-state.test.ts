...
import { ProjectRepository, TaskRepository, MemoryRepository, WorkflowRepository, ConversationRepository } from "../domain/repos.js";
...
import { PROMPT_HISTORY_INDEX, PROMPT_HISTORY_DIR, parsePromptHistory, renderPromptHistory } from "../github/prompt-history.js";
import { ProjectStateGenerator } from "../agents/state-generator.js";
import { ProjectFilesService, parseMatter, matter, PROJECT_FILE, MEMORY_FILE, CONTEXT_FILE, RUNTIME_CONTEXT_FILE, renderAgentFile, renderMemoryFile, renderProjectFile } from "../github/project-files.js";
import { localId, parseSkillFile, renderSkillFile, renderRulesFile, RULES_FILE } from "../github/state-codec.js";
...
const ref = (project = p) => { const [owner, name] = project.configRepo.split("/"); return { owner, name }; };
const get = async (path: string, project = p) => (await gh.getFile(ref(project), path, project.branch))!.content;
const edit = async (path: string, content: string, project = p) => gh.commit(ref(project), project.branch, "Human repository edit", [{ path, content }]);
const body = "# Design\n\nمتن فارسی 🎯\n\n## Risks\nKeep this entire section.\n\n### Details\n```ts\nexport const n = 1;\n```\n\n" + "Long evidence. ".repeat(600);
const memory = (project = p): MemoryEntry => ({ id: localId(project.id, "memory", "decision\0shared/key"), projectId: project.id, type: "decision", scope: "task", key: "shared/key", content: body, tags: ["architecture", "فارسی"], refs: ["task-a", "https://example.test/spec"], source: "human", version: 7, createdAt: project.createdAt, updatedAt: project.updatedAt });
const customSkill = (slug: string, over: Partial<Skill> = {}): Skill => ({ id: "external-skill", slug, name: "Repository skill", description: "Only in Git", category: "project", instructions: "# Exact knowledge\n\nUse the existing session contract.\n", version: "7.2.0", enabled: true, builtIn: false, tools: [], dependencies: [], compatibleAgentTypes: ["*"], metadata: { author: "human" }, createdAt: p.createdAt, updatedAt: p.updatedAt, ...over });
async function server() { app ??= (await buildServer(c)).app; await app.ready(); return app; }

beforeEach(async () => {
  fx = freshDb(); c = new Container(); await c.ensureSeed();
  gh = c.github as MockGitHubService;
  p = await c.agentManager.createProject({ name: "Canonical", configRepo: "acme/canonical", description: "A TypeScript session service", capabilities: { languages: ["typescript"], frameworks: ["react"] } });
});
afterEach(async () => { vi.restoreAllMocks(); c.githubAutomation.stop(); await app?.close(); app = undefined; await new Promise<void>((r) => setImmediate(r)); fx.cleanup(); });

async function model(name = "author", respond?: (req: ChatRequest) => string | Promise<string>) {
  const provider = c.providerRepo.create({ name, type: "openai", baseUrl: "https://unused.invalid/v1", authType: "none", apiFormat: "openai", rateLimitPerMinute: 100, timeoutMs: 1000, defaultTemperature: 0.2, maxTokensDefault: 4500, active: true });
  const m = c.modelRepo.create({ providerId: provider.id, modelId: name, displayName: name, contextWindow: 128000, inputCostPer1k: 0.01, outputCostPer1k: 0.02, capabilities: { code: true, reasoning: true, structuredOutput: true, tools: false, streaming: false, vision: false }, active: true, priority: 1, fallbackPriority: 1, tags: [] });
  const requests: ChatRequest[] = [];
  const runtime: IModelProvider = { id: provider.id, type: "openai", name, health: async () => true, listModels: async () => [], resolveApiKey: () => undefined,
    chat: async (req) => {
      requests.push(req);
      const entries = JSON.parse(req.messages[1].content.split("--- Current operation ---\n").at(-1)!) as Record<string, { fields: Record<string, unknown> }>;
      const result = Object.fromEntries(Object.entries(entries).map(([key, spec]) => [key, Object.fromEntries(Object.keys(spec.fields).map((field) => [field, field === "rules" ? ["Use the documented session contract; do not invent history."] : `AI ${key} ${field}: use the existing session contract and verify changes.`]))]));
      return { content: respond ? await respond(req) : JSON.stringify(result), finishReason: "stop", usage: { inputTokens: 10, outputTokens: 20, totalTokens: 30 }, costUsd: 0, modelId: req.modelId, providerId: provider.id };
    },
  };
  c.providerRegistry.register(runtime); return { model: m, requests };
}
async function copyPartial(name: string, omit: string[], defaultModelId?: string) {
  const state = await c.projectFiles.pull(p);
  const files = [...state.contents].filter(([path]) => !omit.includes(path)).map(([path, content]) => ({ path, content }));
...
describe("canonical CodeVia definitions", () => {
  it("writes complete versioned definitions and explicitly labels offline initialization", async () => {
    ...
    expect(state.files).toEqual(expect.arrayContaining([PROJECT_FILE, MEMORY_FILE, CONTEXT_FILE, RULES_FILE, "CodeVia/skills/testing.md"]));
    const skill = c.skillRepo.findBySlug("testing", p.id)!;
    expect(parseSkillFile(await get("CodeVia/skills/testing.md"))).toMatchObject({ instructions: skill.instructions, dependencies: skill.dependencies, version: skill.version, enabled: true });
  ...
  it("does not call AI or commit when all definitions already exist", async () => {
    ...
    const generate = vi.spyOn(ProjectStateGenerator.prototype, "generate"), commit = vi.spyOn(gh, "commit");
    await c.agentManager.onboardProject(p.id); await c.agentManager.onboardProject(p.id);
  ...
  it("preserves custom prompts, models, tools, permissions, limits, version and disabled agents", async () => {
    ...
    const changed: Agent = { ...a, systemPrompt: "HUMAN RESEARCH PROMPT", projectPrompt: "Custom context", enabled: false, tools: [], permissions: [], memorySources: ["decision"], models: { primary: "do-not-replace", fallbacks: [], specialized: {} }, maxIterations: 2, timeoutMs: 9000, tokenBudget: 1234, version: 29 };
    const file = renderAgentFile(changed); await edit(a.configPath!, file);
    ...
    for (const key of ["systemPrompt", "projectPrompt", "enabled", "tools", "permissions", "models", "maxIterations", "timeoutMs", "tokenBudget", "memorySources", "version"] as const) expect(restored[key]).toEqual(changed[key]);
  });
  ...
  it("discovers a skill placed in Git automatically through the project catalog API", async () => {
    ...
    expect(list.find((s) => s.slug === skill.slug)).toMatchObject({ projectId: p.id, instructions: skill.instructions, dependencies: [] });
  ...
  it("detects conflicting writes even if a different reader refreshed the shared cache", async () => {
    ...
    await edit(old.configPath!, human); await c.agentManager.readProject(p.id);
    await expect(c.projectFiles.syncAgents(p, [{ ...old, systemPrompt: "Stale request" }])).rejects.toMatchObject({ statusCode: 409 });
  ...
  it("does not overwrite project-owned context when execution updates its entity registry", async () => {
    ...
    await syncProjectContext({ files: c.projectFiles, github: gh, project: p, entries: [{ entity: "Session", path: "src/session.ts", agentType: "backend-developer", subtaskId: "task-1", at: p.updatedAt }] });
  ...
  it("prevents two local projects from competing over one canonical folder", async () => {
    await expect(c.agentManager.createProject({ description: "Session service", name: "Conflicting project", configRepo: p.configRepo })).rejects.toMatchObject({ statusCode: 409 });
  });
});
...
describe("lossless memory and restore", () => {
  it("round-trips nested headings, fences, Persian, references, scope, versions and >4K content", async () => {
    const entry = memory(); await edit(MEMORY_FILE, renderMemoryFile([entry]));
    await c.agentManager.readProject(p.id);
  ...
  it("isolates identical memory keys across two project repositories", async () => {
    const b = await c.agentManager.createProject({ description: "Session service", name: "Second", configRepo: "acme/second" });
    const aStore = new CodeViaMemoryStore(c.projectFiles, p), bStore = new CodeViaMemoryStore(c.projectFiles, b);
    await Promise.all([aStore.update({ ...memory(), content: "A" }), bStore.update({ ...memory(b), content: "B" })]);
    expect(c.memoryRepo.byProject(p.id)).toHaveLength(1); expect(c.memoryRepo.byProject(b.id)).toHaveLength(1);
    expect(await aStore.get("decision", "shared/key")).toBe("A"); expect(await bStore.get("decision", "shared/key")).toBe("B");
  ...
  it("serializes concurrent appends without losing either complete memory record", async () => {
    const a = new CodeViaMemoryStore(c.projectFiles, p), b = new CodeViaMemoryStore(c.projectFiles, p);
    await Promise.all([a.append({ ...memory(), content: "First", refs: ["one"] }), b.append({ ...memory(), content: "Second", refs: ["two"] })]);
    const entry = c.memoryRepo.byProject(p.id)[0];
    expect(entry.content).toContain("First"); expect(entry.content).toContain("Second"); expect(entry.version).toBe(2); expect(entry.refs).toEqual(["one", "two"]);
  });

  it("restores complete definitions and histories into a genuinely separate empty database", async () => {
    const task = c.agentManager.createTask({ projectId: p.id, title: "Inspect session", description: "Check authentication", agentType: "research" });
    await c.agentManager.runTask(task.id);
    await edit(MEMORY_FILE, renderMemoryFile([memory()]));
    const conv = c.conversationRepo.create({ projectId: p.id, userId: "local-user", source: "web", title: "Design review", activeAgentId: c.agentRepo.byType(p.id, "research")!.id, messages: [{ id: "message-1", role: "user", content: "Keep the existing API", createdAt: p.createdAt }] });
    await c.projectFiles.syncConversation(c.projectRepo.findById(p.id)!.data, conv);
    const db = new Db(":memory:");
    try {
      const repositories = { projectRepo: new ProjectRepository(db), agentRepo: new AgentRepository(db), taskRepo: new TaskRepository(db), memoryRepo: new MemoryRepository(db), skillRepo: new SkillRepository(db), workflowRepo: new WorkflowRepository(db), runRepo: new RunRepository(db), conversationRepo: new ConversationRepository(db) };
      ...
      expect(new SkillRegistry(repositories.skillRepo).catalog(repositories.projectRepo.findById(p.id)!.data).length).toBeGreaterThan(8);
      ...
      expect(repositories.taskRepo.findById(task.id)!.data).toMatchObject({ status: "succeeded", correlationId: task.correlationId });
...
describe("fail-closed repository boundary", () => {
  it.each([401, 403, 500])("does not regenerate or change cached state after GitHub %i", async (status) => {
    const before = c.agentRepo.byProject(p.id); const generate = vi.spyOn(ProjectStateGenerator.prototype, "generate");
    ...
    expect(c.agentRepo.byProject(p.id)).toEqual(before); expect(generate).not.toHaveBeenCalled();
  });

  it("rejects an incomplete tree rather than initializing over hidden files", async () => {
    await edit("README.md", "New revision");
    vi.spyOn(gh, "listFiles").mockResolvedValue(Array.from({ length: 8000 }, (_, i) => ({ path: `CodeVia/unknown-${i}.md`, type: "blob" as const, size: 0, sha: String(i) })));
  ...
  it("rejects a listed but unreadable file", async () => {
    ...
    vi.spyOn(gh, "getFile").mockImplementation(async (r, path, branch) => path === MEMORY_FILE ? undefined : original(r, path, branch));
  ...
  it.each(["memory", "agent", "schema"])("validates the entire %s snapshot before changing any cache", async (kind) => {
    const old = c.agentRepo.byType(p.id, "research")!;
    const files = [{ path: old.configPath!, content: renderAgentFile({ ...old, name: "Should not partially hydrate" }) }];
    if (kind === "memory") files.push({ path: MEMORY_FILE, content: matter({ schemaVersion: 2, entries: "not an array" }, "") });
  ...
  it("does not acknowledge failed commits as saved settings", async () => {
    ...
    const result = await srv.inject({ method: "PATCH", url: `/projects/${p.id}`, payload: { description: "Uncommitted" } });
  ...
  it("excludes structured credentials and refuses recognizable plaintext credentials", async () => {
    const scoped = { ...p, ownerId: "local-account", githubConnection: { kind: "server-token" } as Project["githubConnection"], settings: { ...p.settings, metadata: { apiKey: "hidden-key", nested: { accessToken: "hidden-token", botToken: "hidden-bot", secretAccessKey: "hidden-aws", maxTokens: 5000, tokenBudget: 1000 } } } };
    const file = renderProjectFile(scoped, c.agentRepo.byProject(p.id), [], []);
    expect(file).not.toContain("hidden-key"); expect(file).not.toContain("hidden-token"); expect(file).not.toContain("hidden-bot"); expect(file).not.toContain("hidden-aws"); expect(file).not.toContain("local-account");
    expect(file).toContain("maxTokens"); expect(file).toContain("tokenBudget");
...
describe("missing-only AI generation", () => {
  it("authors initial project text with the selected model, accounts for usage and never grants capabilities", async () => {
    ...
    const next = await c.agentManager.createProject({ description: "Session service", name: "AI authored", configRepo: "acme/ai-authored", defaultModelId: selected.model.id, capabilities: { agentTypes: ["research"] } });
  ...
  it("fills only missing agent/skill files in an existing copied folder and reuses them on retry", async () => {
    ...
    const kept = await copyPartial("partial", ["CodeVia/agents/backend-developer.md", "CodeVia/skills/testing.md"], selected.model.id);
    const next = await c.agentManager.createProject({ description: "Session service", name: "Reuse folder", configRepo: "acme/partial" });
    ...
    await c.agentManager.onboardProject(next.id); expect(selected.requests).toHaveLength(1);
    await c.agentManager.syncProjectState(next.id); await c.agentManager.onboardProject(next.id);
  ...
  it("treats tombstones and disabled definitions as intentional, not as missing material", async () => {
    const srv = await server(); const a = c.agentRepo.byType(p.id, "backend-developer")!, s = c.skillRepo.findBySlug("testing", p.id)!;
    ...
    const selected = await model();
    await c.agentManager.onboardProject(p.id);
    expect(selected.requests).toHaveLength(0); expect(c.agentRepo.findById(a.id)).toBeUndefined();
  ...
  it("refuses missing material on a real connection without a model instead of writing simulated definitions", async () => {
    ...
    await expect(c.agentManager.createProject({ description: "Session service", name: "No model", configRepo: "acme/no-model" })).rejects.toThrow(/active AI model/);
  ...
  it("does not commit a partially authored batch when the model response is malformed", async () => {
    ...
    await expect(c.agentManager.createProject({ description: "Session service", name: "Malformed", configRepo: "acme/malformed" })).rejects.toThrow();
  ...
  it("stops before a second model call when the configured initialization call budget is exhausted", async () => {
    const selected = await model();
    await expect(c.agentManager.createProject({ description: "Session service", name: "One call", configRepo: "acme/one-call", defaultModelId: selected.model.id, settings: { ...p.settings, budget: { ...p.settings.budget, maxCallsPerRun: 1 } } })).rejects.toThrow(/Budget exceeded/);
    expect(selected.requests).toHaveLength(1);
  ...
  it("aborts a slow authoring call and never commits its late response", async () => {
    ...
    const selected = await model("slow", () => new Promise<string>((resolve) => { release = resolve; }));
    await copyPartial("slow", ["CodeVia/skills/testing.md"], selected.model.id);
    const path = PROJECT_FILE, source = await gh.getFile({ owner: "acme", name: "slow" }, path, "main");
    const parsed = parseMatter(source!.content); ((parsed.data.definition as Project).settings.budget).maxDurationMs = 100;
    await gh.commit({ owner: "acme", name: "slow" }, "main", "Set budget", [{ path, content: matter(parsed.data, parsed.body.replace(/^\n/, "")) }]);
    await expect(c.agentManager.createProject({ description: "Session service", name: "Slow", configRepo: "acme/slow" })).rejects.toThrow(/duration/);
    expect(selected.requests[0].signal?.aborted).toBe(true); release("{}");
...
describe("repository-backed editing and reuse entry points", () => {
  it("writes and reloads a second agent of the same type without a path collision", async () => {
    ...
    const response = await srv.inject({ method: "POST", url: "/agents", payload: { projectId: p.id, type: "research", name: "Independent researcher", systemPrompt: "Human prompt", enabled: false, tools: [], permissions: [] } });
    expect(response.statusCode).toBe(201);
    const agent = response.json() as Agent;
    expect(agent.configPath).not.toBe(original.configPath);
    await c.agentManager.onboardProject(p.id);
    expect(c.agentRepo.findById(agent.id)?.data).toMatchObject({ enabled: false, systemPrompt: "Human prompt", tools: [], permissions: [] });
  ...
  it("uses AI for a new skill with omitted instructions but leaves supplied text untouched", async () => {
    ...
    const srv = await server();
    const generated = (await srv.inject({ method: "POST", url: "/skills", payload: { projectId: p.id, slug: "sessions", name: "Session contract" } })).json() as Skill;
    expect(generated.instructions).toContain("AI skill:sessions");
    expect(selected.requests).toHaveLength(1);
    const manual = (await srv.inject({ method: "POST", url: "/skills", payload: { projectId: p.id, slug: "manual", name: "Manual", instructions: "EXACT HUMAN KNOWLEDGE" } })).json() as Skill;
    expect(manual.instructions).toBe("EXACT HUMAN KNOWLEDGE"); expect(selected.requests).toHaveLength(1);
  ...
  it("authors a skill for project technology that has no built-in template", async () => {
    const selected = await model();
    const next = await c.agentManager.createProject({ name: "Rust service", description: "Typed session API", configRepo: "acme/rust", defaultModelId: selected.model.id, capabilities: { languages: ["rust"], frameworks: ["axum"] } });
  ...
  it("persists memory create, edit, rename and delete through the API", async () => {
    const srv = await server();
    const created = (await srv.inject({ method: "POST", url: "/memory", payload: { projectId: p.id, type: "decision", key: "before", content: body, refs: ["source"] } })).json() as MemoryEntry;
    const edited = (await srv.inject({ method: "PATCH", url: `/memory/${created.id}`, payload: { key: "after", tags: ["updated"] } })).json() as MemoryEntry;
    expect(edited.id).not.toBe(created.id); expect(edited.content).toBe(body); expect(edited.version).toBe(2);
    ...
    await c.agentManager.onboardProject(p.id); expect(c.memoryRepo.byProject(p.id)).toEqual([]);
  });

  it("persists conversation messages and workflow deletion instead of reviving them on Pull", async () => {
    const srv = await server();
    const conv = (await srv.inject({ method: "POST", url: "/conversations", payload: { projectId: p.id, title: "Continuity", userId: "local-user" } })).json();
    const added = await srv.inject({ method: "POST", url: `/conversations/${conv.id}/messages`, payload: { role: "user", content: "Remember the existing contract" } });
    expect(added.statusCode).toBe(200);
    c.conversationRepo.deleteById(conv.id); await c.agentManager.readProject(p.id);
    ...
    await c.agentManager.onboardProject(p.id); expect(c.workflowRepo.findById(w.id)).toBeUndefined();
    expect((await srv.inject({ method: "DELETE", url: `/conversations/${conv.id}` })).statusCode).toBe(200);
    await c.agentManager.readProject(p.id); expect(c.conversationRepo.findById(conv.id)).toBeUndefined();
  });

  it("saves task edits/deletes and does not resurrect an intentionally deleted task", async () => {
    const srv = await server();
    const task = (await srv.inject({ method: "POST", url: "/tasks", payload: { projectId: p.id, title: "Old" } })).json();
    expect((await srv.inject({ method: "PATCH", url: `/tasks/${task.id}`, payload: { title: "New title" } })).statusCode).toBe(200);
    ...
    await c.agentManager.readProject(p.id); expect(c.taskRepo.findById(task.id)).toBeUndefined();
  });
  ...
  it("preserves raw legacy memory before the first v2 update", async () => {
    ...
    const store = new CodeViaMemoryStore(c.projectFiles, p);
    await store.update({ type: "knowledge", key: "new-note", scope: "project", content: "A new finding", refs: [], tags: [] });
...
describe("state webhook loop guard", () => {
  ...
  it("refreshes state-only webhook changes without enqueuing a new agent task", async () => {
    ...
    const result = await c.githubAutomation.handle({ id: "event-state", name: "github.push", correlationId: "state-loop", timestamp: p.createdAt, payload: { body: { repository: { full_name: p.configRepo }, ref: "refs/heads/main", commits: [{ added: [], modified: ["CodeVia/memory.md"], removed: [] }] } } } as Parameters<typeof c.githubAutomation.handle>[0]);
    expect(result).toEqual([]); expect(c.taskRepo.byProject(p.id)).toHaveLength(before);
  });
});


it("validates manual writes before committing, leaving the previous valid definition usable", async () => {
  const srv = await server(); const a = c.agentRepo.byType(p.id, "research")!;
  ...
  expect(result.statusCode).toBe(422); expect(await get(a.configPath!)).toBe(previous);
  await c.agentManager.readProject(p.id); expect(c.agentRepo.findById(a.id)?.data.skills).toEqual(a.skills);
});

it("reads ordinary YAML blocks as well as generated JSON-valued front matter", () => {
  const text = "---\nname: Human edit\nrules:\n  - Preserve interfaces\n  - Keep tests deterministic\ninstructions: |\n  # Steps\n  Keep the full paragraph.\n---\n\nBody";
  expect(parseMatter(text).data).toMatchObject({ rules: ["Preserve interfaces", "Keep tests deterministic"], instructions: "# Steps\nKeep the full paragraph.\n" });
  expect(() => parseMatter('---\nname: "unterminated\n---\n')).toThrow();
});


it("rebinds execution-history references when the folder is copied into another project", async () => {
  const task = c.agentManager.createTask({ projectId: p.id, title: "Inspect API", description: "Check session contract", agentType: "research" });
  await c.agentManager.runTask(task.id);
  await copyPartial("history-copy", []);
  const next = await c.agentManager.createProject({ name: "History copy", description: "Reusing the existing folder", configRepo: "acme/history-copy" });
  ...
  expect(run.projectId).toBe(next.id); expect(restored.result?.runId).toBe(run.id);
...
describe("prompt history is repository state, not a local-only index", () => {
  ...
  it("persists baseline, edits, diff and restore and reloads versions after the index is wiped", async () => {
    ...
    expect((await srv.inject({ method: "PATCH", url: `/agents/${a.id}`, payload: { systemPrompt: "NEW OBSERVED PROMPT", projectPrompt: "وظیفهٔ اختصاصی", note: "human edit" } })).statusCode).toBe(200);
    const persisted = parsePromptHistory(await get(historyPath()));
    expect(persisted.versions.map((v) => v.version)).toEqual([1, 2]);
    expect(persisted.versions[1]).toMatchObject({ systemPrompt: "NEW OBSERVED PROMPT", projectPrompt: "وظیفهٔ اختصاصی", note: "human edit" });
    c.promptVersionRepo.deleteByProject(p.id);
    const versions = (await srv.inject({ method: "GET", url: `/agents/${a.id}/prompt-versions` })).json();
    expect(versions).toHaveLength(2); expect(versions[1].current).toBe(true);
    const diff = await srv.inject({ method: "GET", url: `/agents/${a.id}/prompt-versions/diff?from=1&to=2` });
    expect(diff.statusCode).toBe(200); expect(diff.json().summary.added).toBeGreaterThan(0);
    expect((await srv.inject({ method: "POST", url: `/agents/${a.id}/prompt-versions/1/restore`, payload: {} })).statusCode).toBe(200);
    expect(parsePromptHistory(await get(historyPath())).versions[2]).toMatchObject({ version: 3, derivedFrom: 1, source: "restore", systemPrompt: baseline.systemPrompt });
  ...
  it("restores complete prompt versions into a separate empty database", async () => {
    ...
    const db = new Db(":memory:");
    try {
      const repositories = { projectRepo: new ProjectRepository(db), agentRepo: new AgentRepository(db), taskRepo: new TaskRepository(db), memoryRepo: new MemoryRepository(db), promptVersionRepo: new PromptVersionRepository(db) };
      const files = new ProjectFilesService({ github: gh, repositories, transaction: (fn) => db.tx(fn) });
      await files.restore(p);
  ...
  it("migrates genuine local-only versions without calling AI or replacing existing definitions", async () => {
    ...
    gh.seedRepo("acme", "legacy-history", { files: [...source.contents].filter(([path]) => !path.startsWith("CodeVia/prompts/")).map(([path, content]) => ({ path, content })) });
    const legacy = { ...p, id: "legacy-history-project", slug: "legacy-history", configRepo: "acme/legacy-history", repositories: [{ repo: "acme/legacy-history", branch: "main", role: "primary" as const, isConfigRepo: true }] };
    ...
    const definition = await get(a.configPath!, legacy), generate = vi.spyOn(ProjectStateGenerator.prototype, "generate");
    ...
    expect(await get(a.configPath!, legacy)).toBe(definition); expect(generate).not.toHaveBeenCalled();
  ...
  it("treats Git as authoritative over a stale prompt history cache, without activating an old version", async () => {
    const a = c.agentRepo.byType(p.id, "research")!, original = await get(a.configPath!);
    ...
    expect(versions[0].current).toBe(false); expect(await get(a.configPath!)).toBe(original);
  });
  ...
  it("does not invent or recreate versions in an intentionally empty or removed history", async () => {
    ...
    expect((await srv.inject({ method: "PATCH", url: `/agents/${a.id}`, payload: { systemPrompt: "Explicit new change" } })).statusCode).toBe(200);
  ...
  it("does not retain an unpublished history version after a Git write failure", async () => {
    const srv = await server(); const a = c.agentRepo.byType(p.id, "research")!, old = await get(historyPath());
    const versions = c.promptVersionRepo.forAgent(a.id);
    vi.spyOn(gh, "commit").mockRejectedValue(new Error("permission denied"));
    expect((await srv.inject({ method: "PATCH", url: `/agents/${a.id}`, payload: { systemPrompt: "MUST NOT BECOME HISTORY" } })).statusCode).toBe(502);
    expect(await get(historyPath())).toBe(old); expect(c.promptVersionRepo.forAgent(a.id)).toEqual(versions);
  ...
  it("validates history namespaces before changing any cached records", async () => {
    const a = c.agentRepo.byType(p.id, "research")!, versions = c.promptVersionRepo.forAgent(a.id);
    const history = parsePromptHistory(await get(historyPath()));
  ...
  it("rebinds version identities when a repository folder is copied", async () => {
    const a = c.agentRepo.byType(p.id, "research")!, old = c.promptVersionRepo.forAgent(a.id);
    await copyPartial("prompt-copy", []);
    const next = await c.agentManager.createProject({ name: "Prompt copy", description: "Reuse", configRepo: "acme/prompt-copy" });
    const copied = c.agentRepo.byType(next.id, "research")!, versions = c.promptVersionRepo.forAgent(copied.id);
    ...
    await c.agentManager.syncProjectState(next.id); await c.agentManager.readProject(next.id);
...
describe("automatic discovery completes only needed skill dependencies", () => {
  it("authors absent dependencies of an enabled Git-added skill before it is attached", async () => {
    ...
    const existing = renderSkillFile(skill); await edit("CodeVia/skills/new-git-skill.md", existing);
    await c.agentManager.onboardProject(p.id);
    expect(selected.requests).toHaveLength(1);
    ...
    await c.agentManager.onboardProject(p.id); expect(selected.requests).toHaveLength(1);
  });
