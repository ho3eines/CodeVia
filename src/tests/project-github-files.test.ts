...
beforeEach(async () => {
  ...
  project = await c.agentManager.createProject({
    name: "OAuth files",
    description: "Project-owned browsing",
    configRepo: "acme/oauth-files",
  });
...
describe("project-owned GitHub file browsing", () => {
  it("lists files through the promoted OAuth connection and honors the requested branch", async () => {
    ...
    const res = await app!.inject({
      method: "GET",
      url: `/projects/${project.id}/files?branch=feature`,
      headers: asOwner,
    });
  ...
  it("reads file content through the same connection used for canonical state", async () => {
    ...
    const res = await app!.inject({
      method: "GET",
      url: `/projects/${project.id}/file?path=${remoteFile}&branch=feature`,
      headers: asOwner,
    });
    expect(res.statusCode, res.body).toBe(200);
    expect(res.json()).toMatchObject({
      path: remoteFile,
      content: "Only the project's OAuth repository contains this file.\n",
      sha: "remote-blob",
    });
  ...
  it("rejects the mock recovery write mode for a real OAuth connection", async () => {
    await expect(c.agentManager.syncProjectState(project.id, undefined, { recoverMissingMock: true })).rejects.toThrow(
      /mock recovery cannot write to real GitHub/,
    );
    expect(fetcher).not.toHaveBeenCalled();
  });

  it.each([401, 404, 503])(
    "fails closed on real GitHub HTTP %i without seeding or replacing cached definitions",
    async (status) => {
