...
import { createMockTelegram } from "../integrations/telegram.js";
import { freshDb } from "./test-helpers.js";

const scope = {
  inputs: { enabled: false, retries: 2, tag: "v1", list: [3] },
  outputs: { "backend-unit": { status: "succeeded", data: { branch: "feature/task" } } },
};
describe("workflow data expressions (no JavaScript evaluation)", () => {
  ...
  it.each([
    ...
    "inputs.retries + 1",
    "inputs['prototype']",
  ])("rejects unsafe/unsupported expression %s", (expression) => {
      ...
      resolveWorkflowInput(
        {
          ...
          values: "{{ inputs.list }}",
          message: 'commit {{ outputs["backend-unit"].data.branch }}',
        },
        scope,
      ),
    ).toEqual({ retries: 2, values: [3], message: "commit feature/task" });
    expect(() => resolveWorkflowInput({ branch: "{{ outputs.missing.branch }}" }, scope)).toThrow(
...
let project: Project;
let task: Task;
const node = (id: string, type: WorkflowNode["type"] = "tool", config: Record<string, unknown> = {}): WorkflowNode => ({
  id,
  name: id,
  type,
  config: type === "tool" ? { tool: "probe", input: { label: id }, ...config } : config,
  retries: 0,
});
const link = (from: string, to: string, condition?: string): WorkflowEdge => ({
  from,
  to,
  ...(condition ? { condition } : {}),
});
...
const fakeRun = (request: Parameters<typeof c.agentRunner.run>[0], summary = "done"): Run => ({
  id: `run-${String((request.task.input.workflow as { nodeId: string }).nodeId)}`,
  taskId: task.id,
  projectId: project.id,
...
beforeEach(async () => {
  ...
  project = await c.agentManager.createProject({
    name: "Workflow App",
    configRepo: "acme/workflows",
    description: "A web app",
  });
  task = c.agentManager.createTask({
    projectId: project.id,
    title: "Add login",
    description: "Implement a session API and login page",
  });
});
afterEach(async () => {
  vi.restoreAllMocks();
...
function probe(
  execute = vi.fn(async (_ctx: unknown, input: Record<string, unknown>) => ({
    ok: true,
    output: String(input.label),
    data: input,
  })),
) {
  c.toolRegistry.register({
    name: "probe",
    dangerous: false,
    permissions: [],
    timeoutMs: 5000,
...
describe("dependency-driven workflow execution", () => {
  it("executes all roots, not just the first connected component", async () => {
    const call = probe();
    const result = await c.workflowEngine.run(
      workflow([node("a"), node("b"), node("c")], [link("a", "c")]),
      project,
      task,
    );
  ...
  it.each(["cycle", "unreachable cycle", "missing node", "duplicate id"])(
    "rejects %s before any tool side effect",
    async (kind) => {
      const call = probe();
      const nodes = [node("a"), node("b"), ...(kind === "unreachable cycle" ? [node("root")] : [])];
      if (kind === "duplicate id") nodes.push(node("a"));
      const edges = kind.includes("cycle")
        ? [link("a", "b"), link("b", "a")]
        : kind === "missing node"
          ? [link("a", "absent")]
          : [];
  ...
  it("fans out concurrently, waits for both sides of a diamond and runs the join once", async () => {
    ...
    const call = probe(
      vi.fn(async (_ctx, input) => {
        if (input.label === "right") {
          started();
    ...
    const graph = workflow(
      [node("fork", "parallel"), node("left"), node("right"), node("join")],
      [link("fork", "left"), link("fork", "right"), link("left", "join"), link("right", "join")],
    );
  ...
  it("evaluates a false condition and joins the selected branch without waiting forever for the skipped one", async () => {
    const call = probe();
    const graph = workflow(
      [node("condition", "condition", { expression: "inputs.enabled" }), node("yes"), node("no"), node("join")],
      [link("condition", "yes", "pass"), link("condition", "no", "fail"), link("yes", "join"), link("no", "join")],
    );
  ...
  it("does not execute descendants of a failed node (independent diagnostics still run)", async () => {
    ...
    const result = await c.workflowEngine.run(
      workflow([node("broken"), node("write"), node("diagnostic")], [link("broken", "write")]),
      project,
      task,
    );
    expect(result.status).toBe("failed");
  ...
  it("hands predecessor deliverables and resolved inputs to the next agent", async () => {
    const runs = vi
      .spyOn(c.agentRunner, "run")
      .mockImplementation(async (req) =>
        fakeRun(req, req.agent.type === "backend-developer" ? "API_CONTRACT_FROM_BACKEND" : "client"),
      );
    const graph = workflow(
      [
        node("backend", "agent", { agentType: "backend-developer" }),
        node("frontend", "agent", {
          agentType: "frontend-developer",
          input: { contract: "{{ outputs.backend.summary }}" },
        }),
      ],
      [link("backend", "frontend")],
    );
    expect((await c.workflowEngine.run(graph, project, task)).status).toBe("succeeded");
    expect(runs.mock.calls[1][0].task.input).toMatchObject({
      contract: "API_CONTRACT_FROM_BACKEND",
      workflow: { predecessors: { backend: { summary: "API_CONTRACT_FROM_BACKEND" } } },
    });
  });
  ...
  it("serializes parallel writers and establishes backend contracts before the frontend", async () => {
    ...
    const graph = workflow(
      [
        node("fork", "parallel"),
        node("frontend", "agent", { agentType: "frontend-developer" }),
        node("backend", "agent", { agentType: "backend-developer" }),
      ],
      [link("fork", "backend"), link("fork", "frontend")],
    );
    expect((await c.workflowEngine.run(graph, project, task)).status).toBe("succeeded");
    expect(peak).toBe(1);
  ...
  it("fails closed without an approval channel and treats rejection as final failure", async () => {
    ...
    const noChannel = new WorkflowEngine({
  ...
  it("honours cancellation after an approval wait instead of continuing tool nodes", async () => {
    ...
    c.approvalChannel = async () => {
      c.taskRepo.upsert({ ...task, status: "cancelled" }, { projectId: project.id });
      return true;
    };
    const result = await c.workflowEngine.run(
      workflow([node("gate", "approval"), node("write")], [link("gate", "write")]),
      project,
      task,
    );
  ...
  it("resolves real output values in a tool node, including their original types", async () => {
    ...
    const graph = workflow(
      [
        node("source", "tool", { input: { label: "source", value: 4 } }),
        node("next", "tool", { input: { label: "next", received: "{{ outputs.source.data.value }}" } }),
      ],
      [link("source", "next", "outputs.source.ok === true")],
    );
    expect((await c.workflowEngine.run(graph, project, task)).status).toBe("succeeded");
...
describe("workflow implementation integration (Mock GitHub, not real CI)", () => {
  it("verifies the final shared branch and retains all writers' files", async () => {
    const graph = workflow(
      [
        node("backend", "agent", { agentType: "backend-developer" }),
        node("frontend", "agent", { agentType: "frontend-developer" }),
        node("qa", "agent", { agentType: "qa-test" }),
      ],
      [link("backend", "frontend"), link("frontend", "qa")],
    );
  ...
  it("does not attest changes written after QA, even if the earlier QA node passed", async () => {
    const graph = workflow(
      ...
      [link("backend", "qa"), link("qa", "frontend")],
    );
  ...
  it("does not silently call a coding-only workflow fully verified", async () => {
    const result = await c.workflowEngine.run(
      workflow([node("backend", "agent", { agentType: "backend-developer" })], []),
      project,
      task,
    );
    expect(result.status).toBe("failed");
  ...
  it("seeds QA after all writers in both built-in workflows", () => {
    for (const graph of c.workflowRepo.byProject(project.id)) {
      ...
      const lastWriter = Math.max(
        ...graph.nodes.map((n, i) =>
          ["backend-developer", "frontend-developer", "uiux", "database", "documentation"].includes(
            String(n.config.agentType),
          )
            ? i
            : -1,
        ),
...
describe("unimplemented workflow actions cannot report success (A17)", () => {
  const telegramNode = (id = "send"): WorkflowNode => ({
    id,
    name: "Send Telegram",
    type: "telegram",
  ...
  it("sends a telegram node through the configured adapter", async () => {
    ...
    const result = await engine.run(workflow([telegramNode()], []), project, task);
    expect(result.status).toBe("succeeded");
    expect(mock.sent).toHaveLength(1);
    expect(mock.sent[0]).toMatchObject({ chatId: "audit-chat", text: "Audit message" });
  });

  it("fails a telegram node explicitly when no adapter is configured", async () => {
    const engine = new WorkflowEngine({
      agentRepo: c.agentRepo,
      agentRunner: c.agentRunner,
      toolRegistry: c.toolRegistry,
    ...
    const result = await engine.run(workflow([telegramNode()], []), project, task);
    expect(result.status).toBe("failed");
    expect(result.trace.find((r) => r.id === "send")?.status).toBe("failed");
    expect(result.error ?? "").not.toBe("succeeded");
  });

  it.each(["webhook", "trigger"])("fails %s nodes explicitly as unsupported", async (type) => {
    const engine = new WorkflowEngine({
      agentRepo: c.agentRepo,
      agentRunner: c.agentRunner,
      toolRegistry: c.toolRegistry,
    ...
    expect(result.status).toBe("failed");
    expect(result.trace.find((r) => r.id === "n")?.status).toBe("failed");
  });
});
