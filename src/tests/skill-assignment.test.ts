...
import {
  ...
  selectTaskSkills,
} from "../skills/assignment.js";
import { Container } from "../app/container.js";
import { freshDb } from "./test-helpers.js";

const project = (
  capabilities: Parameters<typeof normalizeCapabilities>[0] = {
  ...
  settings: {
    ...
    budget: { maxCallsPerRun: 20, maxTokensPerRun: 20000, maxDurationMs: 120000, maxCostUsdPerRun: 5 },
  },
  active: true,
  createdAt: "",
  updatedAt: "",
...
const custom = (slug: string, extra: Partial<Skill> = {}): Skill => ({
  ...
  slug,
  name: slug,
  description: "Custom project convention",
  category: "general",
  instructions: `BASE_${slug}`,
  dependencies: [],
  compatibleAgentTypes: [],
  tools: ["ungranted-tool"],
  metadata: {},
  version: "2.0",
  enabled: true,
  builtIn: false,
...
describe("task-scoped skill selection", () => {
  it("filters legacy mixed-stack defaults by the project and specialist responsibility", () => {
    ...
    for (const selection of [backend, frontend, database]) {
      ...
      for (const slug of ["csharp", "sqlserver", "blazor"])
        expect(selection.assignments.map((s) => s.slug)).not.toContain(slug);
    }
  });
  ...
  it("injects attached project skills only into compatible agents", () => {
    ...
    const catalog = [
      ...BUILTIN_SKILLS,
      custom("design-kit", { compatibleAgentTypes: ["frontend-developer"], dependencies: ["css"] }),
    ];
    const ui = selectTaskSkills(catalog, p, agent("frontend-developer"), {
  ...
  it("resolves transitive knowledge once, without imposing a different stack", () => {
    ...
    const selection = selectTaskSkills(BUILTIN_SKILLS, p, agent("frontend-developer"), {
      title: "Form",
      description: "",
      input: { skills: ["blazor", "ui-design"] },
    });
    ...
    expect(resolveSkillDependencies(BUILTIN_SKILLS, ["react", "nodejs", "sqlserver"]).map((s) => s.slug)).toEqual([
      "react",
      "nodejs",
      "sqlserver",
    ]);
  ...
  it("adapts instructions independently per duty without mutating the catalog or agent", () => {
    ...
    const select = (title: string, guidance: string) =>
      selectTaskSkills(BUILTIN_SKILLS, p, a, {
        title,
        description: "Server work",
        input: { files: ["src/auth.ts"], skills: ["security"], skillInstructions: { security: guidance } },
      });
  ...
  it("re-evaluates repair guidance and adds compatible testing knowledge", () => {
    const fix = selectTaskSkills(BUILTIN_SKILLS, project(), agent("backend-developer"), {
      title: "Repair session expiry",
      description: "Server fix",
      input: { skills: ["restapi"], fixContext: "src/auth.ts: expected 401, got 200" },
    });
  ...
  it("ignores disabled automatic roots but rejects explicit unavailable, incompatible or wrong-stack selections", () => {
    ...
    for (const slug of ["testing", "missing", "postgresql", "blazor"]) {
      expect(() => selectTaskSkills(catalog, p, a, { title: "T", description: "", input: { skills: [slug] } })).toThrow(
  ...
  it("rejects malformed skill assignments and guidance for unassigned skills", () => {
    for (const input of [
      ...
      expect(() =>
        selectTaskSkills(BUILTIN_SKILLS, project(), agent("backend-developer"), { title: "T", description: "", input }),
      ).toThrow();
  });

  it("fails clearly on missing/disabled prerequisites and dependency cycles", () => {
    expect(() => resolveSkillDependencies([custom("a", { dependencies: ["missing"] })], ["a"])).toThrow(
      /missing or disabled/,
    );
    expect(() =>
      resolveSkillDependencies([custom("a", { dependencies: ["b"] }), custom("b", { enabled: false })], ["a"]),
    ).toThrow(/missing or disabled/);
    expect(() =>
      resolveSkillDependencies([custom("a", { dependencies: ["b"] }), custom("b", { dependencies: ["a"] })], ["a"]),
    ).toThrow(/Circular/);
  });
});
...
describe("skill persistence and onboarding", () => {
  ...
  it("generates stack-specific defaults and preserves manual project/agent skill choices on re-onboard", async () => {
    const p = await c.agentManager.createProject({
      name: "Skill Profile",
      description: "Store",
      configRepo: "acme/skill-profile",
      capabilities: { platforms: ["web"], languages: ["csharp"], frameworks: ["blazor"], databases: ["oracle"] },
    });
    ...
    c.projectRepo.upsert(
      { ...p, settings: { ...p.settings, skills: [...p.settings.skills, "manual-project"] } },
      { key: p.slug },
    );
    c.agentRepo.upsert(
      { ...ui, skills: [...ui.skills.filter((s) => s !== "testing"), "manual-agent"] },
      { projectId: p.id },
    );
    ...
    await c.projectFiles.restore(
      p,
      { projectRepo: c.projectRepo, taskRepo: c.taskRepo, agentRepo: c.agentRepo, memoryRepo: c.memoryRepo },
      { includeTasks: false },
    );
    expect(c.agentRepo.byType(p.id, "frontend-developer")!.generatedSkills).toEqual(next.generatedSkills);
  });

  it("rehydrates a custom skill definition (instructions + dependencies) into the project namespace on restore", async () => {
    const p = await c.agentManager.createProject({
      name: "Skill Restore",
      description: "Store",
      configRepo: "acme/skill-restore",
    });
    const {
      id: _id,
      createdAt: _c,
      updatedAt: _u,
      ...data
    } = custom("audit-company-policy", {
      instructions: "AUDIT_REQUIRED_COMPANY_POLICY",
      dependencies: ["security"],
    });
    const skill = c.skillRepo.create(data);
    c.projectRepo.upsert(
      { ...p, settings: { ...p.settings, skills: [...p.settings.skills, skill.slug] } },
      { key: p.slug },
    );
    // Attach must author the definition under CodeVia/skills/<slug>.md so a later
    // restore can rehydrate it even after the catalog copy is gone.
    await c.agentManager.syncProjectState(p.id);
    c.skillRepo.deleteById(skill.id);
    await c.projectFiles.restore(
      c.projectRepo.findById(p.id)!.data,
      { projectRepo: c.projectRepo, taskRepo: c.taskRepo, agentRepo: c.agentRepo, memoryRepo: c.memoryRepo },
      { includeTasks: false },
    );
    const restored = c.skillRepo.findBySlug(skill.slug, p.id);
    expect(restored?.instructions).toBe("AUDIT_REQUIRED_COMPANY_POLICY");
    expect(restored?.dependencies).toContain("security");
  });
});
