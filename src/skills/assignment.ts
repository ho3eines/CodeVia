...
export function compatibleSkill(skill: Skill, type: Agent["type"]): boolean {
  return (
    !skill.compatibleAgentTypes.length ||
    skill.compatibleAgentTypes.includes("*") ||
    skill.compatibleAgentTypes.includes(type)
  );
}
...
export function availableSkills(catalog: Skill[], project: Project, agent: SkillAgent): Skill[] {
  ...
  const manual = new Set([
    ...
    ...(project.settings?.generatedSkills
      ? project.settings.skills.filter((slug) => !project.settings.generatedSkills!.includes(slug))
      : []),
  ]);
  ...
  const nonNodeBackend = caps.frameworks.some((f) =>
    ["dotnet", "aspnetcore", "django", "fastapi", "flask", "spring", "laravel", "rails"].includes(f),
  );
  ...
  return catalog
    .filter(
      (s) =>
        s.enabled &&
        compatibleSkill(s, agent.type) &&
        (!s.builtIn || !STACK_SKILLS.has(s.slug) || selected.has(s.slug) || manual.has(s.slug)) &&
        (!s.builtIn ||
          !backendRole ||
          !nonNodeBackend ||
          nodeBackend ||
          !["nodejs", "typescript"].includes(s.slug) ||
          manual.has(s.slug)),
    )
    .filter((s) => {
      try {
        resolveSkillDependencies(catalog, [s.slug]);
        return true;
      } catch {
        return false;
      }
...
export function skillSlugs(value: unknown): string[] | undefined {
  ...
  if (
    !Array.isArray(value) ||
    value.length > 16 ||
    value.some((s) => typeof s !== "string" || !/^[a-z0-9][a-z0-9._-]{0,79}$/i.test(s))
  ) {
...
export function skillInstructions(value: unknown): Record<string, string> {
  ...
  if (
    entries.some(
      ([slug, text]) =>
        !/^[a-z0-9][a-z0-9._-]{0,79}$/i.test(slug) || typeof text !== "string" || !text.trim() || text.length > 1200,
    )
  ) {
...
export function selectTaskSkills(
  ...
  for (const slug of explicit ?? []) {
    if (!available.has(slug))
      throw new Error(`Skill ${slug} is unavailable or incompatible with ${agent.type} in this project`);
  }
  const text = task ? `${task.title}\n${task.description}\n${String(task.input.fixContext ?? "")}` : "";
  const suggested = TASK_SIGNALS.filter(([, re]) => re.test(text)).map(([slug]) => slug);
  const roots = [...new Set(explicit ?? [...agent.skills, ...configured, ...suggested])].filter((slug) =>
    available.has(slug),
  );
  ...
  const files = Array.isArray(task?.input.files)
    ? task.input.files.filter((p): p is string => typeof p === "string").slice(0, 5)
    : [];
  return {
    skills: roots,
    assignments: resolved.map((s) => ({
      ...
      source: !roots.includes(s.slug)
        ? "dependency"
        : explicit?.includes(s.slug)
          ? "task"
          : agent.skills.includes(s.slug)
            ? "agent"
            : configured.has(s.slug)
              ? "project"
              : "task",
      guidance: task
        ? [
            ...
            overrides[s.slug] ??
              "Follow the task's acceptance criteria, configured stack and existing interfaces; do not expand its scope.",
