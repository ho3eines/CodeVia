...
(() => {
  ...
  const projectActionBar = (p) => `<div class="action-row">
    ...
    <button class="btn" onclick="projectReonboard(${JSON.stringify(p.id)})">↻ Load / fill missing</button>
    <button class="btn" title="Restore agents, tasks, memory and skills from the CodeVia/ folder in git" onclick="projectPull(${JSON.stringify(p.id)})">⬇ Pull from GitHub</button>
  ...
  on("/runs/:id/console", async (rest) => {
    ...
    $("#content").innerHTML = `
      <div class="overview"><div><h1>Run Console</h1><p class="mono">${esc(c.runId)}</p></div>
        <div class="action-row">${badge(c.status)} ${verificationBadge(c.verification)}<span class="pill">Model: ${esc(c.modelId || "—")}</span><a class="btn" href="#/projects/${esc(c.projectId)}/runs">← Project runs</a><button class="btn" onclick="projectRunTask(${esc(JSON.stringify(c.taskId))})">↻ Retry task</button>${c.status === "failed" || c.error ? `<button class="btn btn-primary" onclick="projectDebugRun(${JSON.stringify(c.runId)})">🐞 Send to debugging agent</button>` : ""}</div></div>
      <div class="stat-grid">
      ...
      <div class="card card-body"><div class="card-title">Execution Steps</div>
        ...
        ${(c.error || (c.steps || []).some((s) => s.status === "failed")) ? `<div class="error-state mt"><h4>What happened</h4><pre>${esc(c.error || "One or more steps failed — see below.")}</pre>${(c.steps || []).filter((s) => s.status === "failed").map((s) => `<div class="meter-row"><span class="lbl">${esc(s.label)}</span><span class="mono">${esc(s.tool || "step")}</span></div>${s.detail ? `<pre>${esc(s.detail)}</pre>` : ""}`).join("")}<div class="field-hint">Suggested: retry once — if it fails again, send the run to the debugging agent for root-cause analysis.</div><div class="flex mt"><button class="btn" onclick="projectRunTask(${esc(JSON.stringify(c.taskId))})">↻ Retry</button><button class="btn btn-primary" onclick="projectDebugRun(${JSON.stringify(c.runId)})">🐞 Send to agent</button></div></div>` : ""}
      </div>`;
  });
