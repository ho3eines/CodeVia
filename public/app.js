...
(() => {
  ...
  window.projectViewTask = async (taskId) => {
    ...
    const brief = t.input?.researchBrief ? `<div class="field"><label>🔎 Research brief <span class="sub">built by the research unit — every subtask works from this</span></label><pre class="mini-pre">${esc(t.input.researchBrief)}</pre></div>` : "";
    const kidsHtml = kids.length ? `<div class="field"><label>🧩 Subtasks <span class="sub">${kids.filter((k) => k.status === "succeeded").length}/${kids.length} done</span></label><div class="table-wrap"><table><tbody>${kids.map((k) => `<tr><td><strong>${esc(k.title)}</strong></td><td class="mono">${esc(k.agentType || "auto")}</td><td>${badge(k.status)}</td><td><button class="btn btn-ghost" onclick="projectViewTask(${JSON.stringify(k.id)})">View</button></td></tr>`).join("")}</tbody></table></div></div>` : "";
    openModal("Task details", `<div class="meter-row"><span class="lbl">Title</span><strong>${esc(t.title)}</strong></div><div class="meter-row"><span class="lbl">Status</span>${badge(t.status)}</div><div class="meter-row"><span class="lbl">Agent</span><span class="mono">${esc(t.agentType || "auto")}</span></div><div class="meter-row"><span class="lbl">Correlation</span><span class="mono">${esc(t.correlationId)}</span></div><div class="field"><label>Description</label><pre class="mini-pre">${esc(t.description || "")}</pre></div>${brief}${kidsHtml}${t.result ? `<div class="field"><label>Result</label>${miniJson(t.result)}</div>` : ""}${t.error ? `<div class="error-state"><h4>Task error</h4><pre>${esc(t.error)}</pre></div>` : ""}<div class="flex mt"><button class="btn btn-primary" onclick="projectRunTask('${esc(t.id)}')">Run</button><button class="btn" onclick="projectCancelTask('${esc(t.id)}')">Cancel</button><button class="btn" onclick="closeModal()">Close</button></div>`, { wide: true });
  };
