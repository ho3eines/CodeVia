...
AgentManager (Orchestrator)
   ...
   └── WorkflowEngine     (dependency-driven DAG — Debug→Backend→Frontend→QA→Review)
```

**AgentRunner** creates the observable Run before preparation, composes configured prompts/skills/rules, and routes budgeted model calls by agent/category. `Implementation` prepares up to five files before one atomic commit; existing files use exact text patches against a complete SHA-pinned snapshot. Autonomous task implementers share one work branch and draft PR per linked repository. QA reads GitHub checks for the actual work commit; failures are patched on that same branch. Model failures and truncated output never turn into live-repository scaffold writes. Cancellation is inherited by subtasks, and tool results/usage survive failed runs.

`run_tests` / `run_build` verify GitHub CI rather than executing arbitrary commands on the platform host. Missing/pending CI is **unverified**; Mock GitHub is explicitly **simulated**, not tested. See [AGENT_EXECUTION.md](AGENT_EXECUTION.md) for setup, limits, permissions and migration of legacy command-based workflows.

The WorkflowEngine validates the entire DAG before executing nodes, supports all roots, evaluates data-only `inputs`/`outputs` conditions, and resolves `{{ ... }}` input references without `eval`. Joins wait for all selected predecessors; failed dependencies block descendants. Independent work can run concurrently, but mutations are serialized within a workflow to protect the shared branch. Downstream agents receive predecessor deliverables and implementation references; QA must cover the final writer SHA. A later write invalidates prior QA evidence. Workflow traces, artifact references and verification are retained on the task.
...
`JobQueue` (backed by `jobs` table) supports idempotent enqueue (correlation-keyed), atomic `UPDATE … RETURNING` claim (oldest first), retry, and stats. `Worker` polls and dispatches job types (`agent.run`, `workflow.run`, `telegram.send`, `notify`, `github.op`). Resilience: retry with exponential backoff, dead-letter after max attempts, idempotency, logging. The worker runs in a separate process on Railway and never blocks the UI thread.
...
- **SSRF / command protection** — built-in build/test tools read GitHub CI evidence for a specific SHA; command/cwd inputs are rejected and no project scripts run on the CodeVia host.
