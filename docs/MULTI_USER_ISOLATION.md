...
| ~~`/runs/:id`, `/tasks/:id`, `/conversations/:id` by id~~ | **Closed (2026-09-12, extended 2026-09-13):** every by-id handler now checks `canAccessEntity()` itself (`src/http/project-access.ts`) in addition to the project-state hook — `/tasks/:id*`, `/runs/:id*`, `/conversations/:id*` (incl. summarize/delete), `/approvals/:id*`, and since the follow-up also **agents, workflows, memory and skills** by-id plus the task/agent/memory/skill *create* routes (`resolveProjectForRequest`). Project-less entities answer 404 for signed-in accounts and stay visible to the demo identity, except intentionally shared rows (marketplace skill templates, platform memory — `detached: "shared"`). Regression: `src/tests/entity-access-gate.test.ts` (10 tests). |
