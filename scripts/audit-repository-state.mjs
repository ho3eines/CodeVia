...
Object.assign(process.env, {
  ...
  TELEGRAM_MODE: "off",
  AUTH_SECRET: "local-repository-audit-fixture-not-a-real-credential",
});
globalThis.fetch = async () => {
  throw new Error("Outbound fetch is forbidden in this audit");
};
...
const { parseMatter, matter, renderAgentFile, renderTaskFile, CONTEXT_FILE, PROJECT_FILE } =
  await import("../src/github/project-files.ts");
...
async function copy(c, p, name, transform = (files) => files) {
  ...
  c.github.seedRepo("audit", name, {
    files: transform([...state.contents].map(([path, content]) => ({ path, content }))),
  });
...
async function research(c, p) {
  const t = c.agentManager.createTask({
    projectId: p.id,
...
await probe(
  "P01",
  "control",
  "Existing definitions are reused without generation",
  "A second onboarding does not call the state author or commit files",
  ["src/agents/project-state.ts"],
  async ({ c, p }) => {
    let writes = 0,
      generates = 0;
    const commit = c.github.commit,
      generate = ProjectStateGenerator.prototype.generate;
    c.github.commit = async function (...args) {
      writes++;
      return commit.apply(this, args);
    };
    ProjectStateGenerator.prototype.generate = async function (...args) {
      generates++;
    ...
    try {
      ...
      c.github.commit = commit;
      ProjectStateGenerator.prototype.generate = generate;
    }
  },
);
...
await probe(
  "P02",
  "control",
  "Git-added skill definitions load automatically",
  "The project catalog reads the full Git definition without requiring a DB seed",
  ["src/http/project-state-hook.ts", "src/skills/registry.ts"],
  async ({ c, p, app }) => {
    const skill = {
      id: "audit-custom",
      slug: "audit-custom",
      name: "Git knowledge",
      description: "Fixture",
      instructions: "UNIQUE_GIT_SKILL_INSTRUCTIONS",
      category: "project",
      enabled: true,
      builtIn: false,
      version: "2.0",
    ...
    await c.github.commit(repo(p), p.branch, "Edit skill", [
      { path: "CodeVia/skills/audit-custom.md", content: renderSkillFile(skill) },
    ]);
    ...
    return {
      ok: found?.instructions === skill.instructions,
      evidence: {
        status: response.statusCode,
        projectScoped: found?.projectId === p.id,
        fullInstructionsRead: found?.instructions === skill.instructions,
      },
    };
  },
);
...
await probe(
  "P03",
  "control",
  "Long structured memory survives a cleared index",
  "Headings, Unicode and references survive a repository round-trip",
  ["src/memory/codevia-store.ts", "src/github/project-state.ts"],
  async ({ c, p }) => {
    ...
    c.memoryRepo.deleteByProject(p.id);
    await c.agentManager.readProject(p.id);
    const restored = c.memoryRepo.byProject(p.id).find((m) => m.key === "roundtrip");
    return {
      ok: restored?.content === content && restored.refs[0] === "reference",
      evidence: {
        inputLength: content.length,
        restoredLength: restored?.content.length,
        exactMatch: restored?.content === content,
      },
    };
  },
);
...
await probe(
  "R01",
  "high",
  "Stored project context must reach the model",
  "The default research and implementation context should include the canonical architecture document",
  ["src/ai/context-engine.ts", "src/agents/context.ts"],
  async ({ c, p }) => {
    const marker = "REPOSITORY_ONLY_ARCHITECTURE_CONSTRAINT_9281";
    const rulesMarker = "POSITIVE_CONTROL_RULE_REACHES_MODEL_7162";
    await c.github.commit(repo(p), p.branch, "Edit architecture", [
      {
        path: CONTEXT_FILE,
    ...
    const config = c.providerRepo
      .findMany()
      .map((r) => r.data)
      .find((v) => v.type === "mock");
    const provider = c.providerRegistry.resolve(config),
      chat = provider.chat,
      requests = [];
    ...
    try {
      ...
      return {
        ok: reachedModel && reachedImplementationContext,
        evidence: {
          storedInGit: (await c.github.getFile(repo(p), CONTEXT_FILE, p.branch)).content.includes(marker),
          modelCalls: requests.length,
          rulesReachedModel,
          reachedModel,
          reachedImplementationContext,
        },
      };
    } finally {
      provider.chat = chat;
...
await probe(
  "R02",
  "high",
  "Completed history must not remain DB-authoritative",
  "A Git edit to a completed task description/run summary should appear in the API without erasing the DB",
  ["src/github/project-state.ts"],
  async ({ c, p, app }) => {
    const task = await research(c, p),
      run = c.runRepo.byTask(task.id)[0];
    const title = "REPOSITORY_EDITED_TASK_TITLE",
      description = "REPOSITORY_EDITED_TASK_DESCRIPTION",
      summary = "REPOSITORY_EDITED_RUN_SUMMARY";
    ...
    const rt = await app.inject({ method: "GET", url: `/tasks/${task.id}` }),
      rr = await app.inject({ method: "GET", url: `/runs/${run.id}` });
    assert.equal(rt.statusCode, 200);
    assert.equal(rr.statusCode, 200);
    return {
      ok: rt.json().title === title && rt.json().description === description && rr.json().summary === summary,
      evidence: {
        terminalStatus: task.status,
        taskApiSeesGitEdit: rt.json().title === title,
        taskDescriptionSeesGitEdit: rt.json().description === description,
        runApiSeesGitEdit: rr.json().summary === summary,
        oldTaskTitleStillUsed: rt.json().title === task.title,
      },
    };
  },
);
...
await probe(
  "R03",
  "high",
  "Workflow tombstones must survive repository copies",
  "A deliberately deleted default workflow must not be regenerated under a new project ID",
  ["src/agents/project-state.ts", "src/github/project-state.ts"],
  async ({ c, p, app }) => {
    ...
    await c.agentManager.onboardProject(p.id);
    assert.equal(
      c.workflowRepo.byProject(p.id).some((w) => w.slug === removed.slug),
      false,
    );
    ...
    return {
      ...
      evidence: {
        tombstonePreserved:
          parseMatter((await c.github.getFile(repo(cloned), path, cloned.branch)).content).data.deleted === true,
        deletedSlug: removed.slug,
...
await probe(
  "R04",
  "high",
  "Deletion should refer to an agent identity, not just a default filename",
  "Deleting a default-role agent after moving its file in Git must remain deleted",
  ["src/agents/project-state.ts", "src/http/routes/agents.ts"],
  async ({ c, p, app }) => {
    const old = c.agentRepo.byType(p.id, "research");
    const movedPath = "CodeVia/agents/custom-research-location.md";
    const cloned = await copy(c, p, "moved-agent", (files) =>
      files.map((f) => (f.path === old.configPath ? { ...f, path: movedPath } : f)),
    );
    const agent = c.agentRepo.byType(cloned.id, "research");
    assert.equal(agent.configPath, movedPath);
    ...
    return {
      ...
      evidence: {
        ...
        replacementPath: replacement?.configPath,
...
await probe(
  "R05",
  "high",
  "Migration must not drop a DB-only legacy agent definition",
  "An old saved agent should be migrated or explicitly rejected before it is deleted, not silently replaced",
  ["src/agents/project-state.ts", "src/github/project-state.ts"],
  async ({ c, p }) => {
    const legacy = {
      ...p,
    ...
    const agent = {
      ...c.agentRepo.byType(p.id, "research"),
    ...
    return {
      ok: afterSecond?.id === agent.id && afterSecond.systemPrompt === agent.systemPrompt,
      evidence: {
        existedInLegacyDb: true,
        presentAfterFirstOnboard: !!afterFirst,
        originalId: agent.id,
        laterId: afterSecond?.id,
        customPromptReused: afterSecond?.systemPrompt === agent.systemPrompt,
        expectedDisabled: true,
        laterEnabled: afterSecond?.enabled,
        expectedPermissions: [],
        laterPermissions: afterSecond?.permissions,
        expectedTokenBudget: 321,
        laterTokenBudget: afterSecond?.tokenBudget,
        archivedPromptVersions: c.promptVersionRepo.forAgent(agent.id).length,
      },
    };
  },
);
...
await probe(
  "R06",
  "high",
  "Legacy workflows require migration too",
  "Custom/disabled legacy workflows should not disappear when the CodeVia schema is initialized",
  ["src/agents/project-state.ts", "src/github/project-state.ts"],
  async ({ c, p }) => {
    const legacy = {
      ...p,
    ...
    return {
      ok: !!found && !found.enabled,
      evidence: {
        workflowExistedInDb: true,
        legacyFileStillExists: !!(await c.github.getFile(
          repo(legacy),
          ".ai-engineering/workflows/custom-disabled-flow.json",
          legacy.branch,
        )),
        foundAfterMigration: !!found,
        resultingSlugs: c.workflowRepo.byProject(legacy.id).map((item) => item.slug),
      },
    };
  },
);
...
await probe(
  "R07",
  "medium",
  "Failed cancellation persistence needs retry/unsynced visibility",
  "Cancellation may stop locally during an outage, but must not permanently pretend Git is in sync",
  ["src/agents/manager.ts", "src/http/routes/tasks-runs.ts"],
  async ({ c, p, app }) => {
    ...
    try {
      ...
      c.github.commit = original;
    }
    const retry = await app.inject({ method: "POST", url: `/tasks/${task.id}/cancel` });
    const statusInGit = parseMatter((await c.github.getFile(repo(p), `CodeVia/tasks/${task.id}.md`, p.branch)).content)
      .data.status;
    return {
      ok: statusInGit === "cancelled" || response.statusCode >= 400 || response.json().repositorySynced === false,
      evidence: {
        cancellationStatus: response.statusCode,
        localStatus: c.taskRepo.findById(task.id).data.status,
        retryStatus: retry.statusCode,
        retryAlreadyFinal: retry.json().alreadyFinal,
        statusInGitAfterRetry: statusInGit,
        explicitUnsyncedFlag: response.json().repositorySynced === false,
      },
    };
  },
);
...
await probe(
  "R08",
  "high",
  "Malformed numeric agent settings must fail closed",
  "Invalid schema-2 budgets and limits should be rejected, not converted to larger defaults",
  ["src/github/project-codec.ts", "src/github/project-state.ts"],
  async ({ c, p }) => {
    const a = c.agentRepo.byType(p.id, "research"),
      encoded = parseMatter(renderAgentFile(a));
    Object.assign(encoded.data, { tokenBudget: "BROKEN", timeoutMs: null, maxIterations: "BROKEN", version: "BROKEN" });
    await c.github.commit(repo(p), p.branch, "Malformed numeric fields", [
      { path: a.configPath, content: matter(encoded.data, encoded.body.replace(/^\n/, "")) },
    ]);
    let rejected = false,
      error;
    try {
    ...
    return {
      ...
      evidence: {
        rejected,
        error,
        readTokenBudget: read.tokenBudget,
        readTimeoutMs: read.timeoutMs,
        readMaxIterations: read.maxIterations,
...
const report = {
  ...
  scope:
    "Repository-first request: persistence, actual consumption, reuse, migration, malformed state and deletion semantics",
  externalServicesUsed: false,
  productSourceModifiedByAudit: false,
