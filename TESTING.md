...
9. **زمینه پروژه (context):** بعد از هر تسک خودگردان، فایل `CodeVia/context.md` معماری + رجیستری موجودیت‌ها را دارد:
   ...
   ```
10. **تداوم (عدم فراموشی):** PR بک‌اند را Merge کن، بعد یه تسک دوم روی همان موضوع بزن (مثلاً «Add login rate limiting») → فایل قبلی **extend** می‌شود (پیاده‌سازی قبلی + TODOهای جدید)، نه بازنویسی.
