...
- **Tests:** `src/tests/multi-user-ownership.test.ts` (۸ تست جدید: پنهان‌بودن پرووایدر/مدلِ دیگران، عملیات گروهی، انتقالِ ردیف مشترک، محافظت از Mock توکار، نمایش همه‌چیز به کاربر دمو، و **عدمِ مسیریابی به پرووایدرِ حساب دیگر** با registry ساختگی). `security-regressions` (A02/A03) با قانون جدید به‌روزرسانی شد.
