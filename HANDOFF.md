...
## 2026-09-14 — رفع «AI نمی‌تواند ریپوزیتوری را ببیند» (404 جعلی در چت پروژه)

- **گزارش کاربر (فارسی):** در چت پروژه، AI جواب داد «من نمی‌توانم کد واقعی پروژه شما را ببینم چون ریپازیتوری `ho3eines/Projects` خطای 404 می‌دهد» و بعد بر اساس README یک تحلیل کلی داد. ریپازیتوری در واقع **public و در دسترس** است (906 فایل، 94 پوشه، `README.md` و `.github/workflows/ci.yml` دارد).
- **ریشهٔ مشکل (اندازه‌گیری‌شده، نه حدس):** `buildRepoBrief()` درخت فایل را با **Contents API پوشه به پوشه** می‌خواند → برای همان ریپازیتوری **۹۹ درخواست پشت‌سرهم ≈ ۱۲ ثانیه** (بازتولید با `LATENCY_MS=120`). بودجهٔ چت **۸ ثانیه** بود (`withTimeout(..., 8000)`) و هر خطا/تأخیر با `.catch(() => "")` به **رشتهٔ خالی** تبدیل می‌شد. یعنی مدل **هیچ** شاهدی از ریپو نمی‌گرفت و **هیچ** توضیحی هم نداشت → محتمل‌ترین بهانه را می‌ساخت: «404». هر پیام چت هم همین ۹۹ درخواست را تکرار می‌کرد (نزدیک به rate limit).
- **ماژول جدید `src/agents/repo-brief.ts`:**
  - `readRepoBrief()` نتیجهٔ **ساختاریافته** می‌دهد: `status: ok|empty|unavailable`، `reason`، `hint`، `errorKind` (`not-found|auth|rate-limit|timeout|too-large|network`)، `files`، `directories` (رول‌آپ پوشه‌های سطح بالا)، `configuredBranch`، `elapsedMs`، `source`.
  - **کش** با کلید `account|owner/name@branch` و TTL از `REPO_BRIEF_TTL_MS` (پیش‌فرض ۱۲۰ ثانیه، حداکثر ۱۲۸ ورودی)؛ `cacheScope` اجباری است تا «خوانده‌شدن/نشدن» یک ریپوی private بین حساب‌ها نشت نکند. `refresh:true` کش را دور می‌زند.
  - **خوددرمانی شاخه:** اگر شاخهٔ ذخیره‌شدهٔ پروژه 404 بدهد، یک بار شاخهٔ پیش‌فرض ریپو (یا `main`/`master`) امتحان می‌شود و `configuredBranch` + راهنمای اصلاح گزارش می‌شود.
  - `repoContextPrompt()` بلوک پرامپت را می‌سازد: با شاهد → همان لیست/README/مانیفست‌ها؛ بدون شاهد → «Repository context: UNAVAILABLE» + **دلیل واقعی** + قانون صریح «هرگز نگو ریپو وجود ندارد یا 404 می‌دهد، هرگز فایل ندیده‌شده را توصیف نکن، هر فرض را فرض بنام».
- **`RealGitHubService.listFiles` بازنویسی شد:** اول **Git Trees API** (`/git/trees/{ref}?recursive=1`) در **یک درخواست**؛ فقط وقتی آن جواب کامل ندهد (404، ریپوی خالی، `truncated`، payload نامعتبر، یا هر خطای شبکه/پراکسی) به **پیاده‌روی Contents** برمی‌گردد و در شکست، خطای اعتبارنامه‌ای (401/403) همان تشخیص دقیق‌تر را نگه می‌دارد. پیام 404 حالا نام ریپو و ref را می‌گوید نه فقط URL خام API.
  - اندازه‌گیری بعد از تغییر روی همان ریپازیتوری واقعی: **۹۹ درخواست / ۱۲ ثانیه → ۵ درخواست / ۰٫۶ ثانیه** (و پیام دوم از کش: صفر درخواست).
- **روت چت (`src/http/routes/conversations.ts`):** `readProjectRepoBrief()` جای `withTimeout + catch("")` را گرفت (خطای خودِ resolve اتصال هم به تشخیص تبدیل می‌شود، نه سکوت)؛ پاسخ JSON فیلد `repoContext` دارد و استریم SSE فریم `repoContext` می‌فرستد تا UI همان چیزی را نشان بدهد که مدل گرفته است.
- **مسیر جدید `GET /projects/:id/repo-status`:** اعتبارنامهٔ فعال (`source`/`login`/`scopes`/`tokenStored`)، به ازای هر مخزن `readable`/`exists`/`branchExists`/`defaultBranch`/`headSha`/`files`/`readme`/`ciWorkflows`/`availableBranches` + `error` و `hint` عملیاتی، بعلاوه همان `brief` که چت می‌گیرد و `healthy` کلی. `?refresh=1` کش را دور می‌زند. (مالکیت مثل بقیهٔ روت‌های پروژه: پروژهٔ دیگری ۴۰۴ است.)
- **UI (`client/app/140-conversations-memory-github.js` + `50-project-detail.js`، باندل با `npm run build:app`):** بنر «Repository context» بالای رشتهٔ چت — هم در صفحهٔ کامل مکالمه و هم در **تب Chat پروژه** (همان جایی که کاربر گزارش داد): تعداد فایل، README ✓/✗، **CI ✓/✗** (نبودِ GitHub Actions یعنی QA فقط `unverified` می‌دهد)، شاخهٔ جایگزین‌شده، هویت اتصال/اسکوپ، و دکمهٔ **↻ Re-check** که با `refresh=1` دوباره می‌خواند. فریم `repoContext` بنر را زنده به‌روز می‌کند.
- **پاسخ به سؤال «کلون → تغییر → تست → push»: ** در `docs/AGENT_EXECUTION.md` بخش جدید «مدل اجرا: بدون کلون محلی» اضافه شد. حلقه وجود دارد ولی روی API گیت‌هاب است: خواندن درخت/محتوا (بدون دیسک)، commit اتمیک روی `agent-task-<id>`، Draft PR، و **تست = Check Runهای GitHub Actions** (نه اجرای `npm test` روی سرور CodeVia — آن یعنی اجرای کد از راه دور روی میزبانی که توکن‌ها و دیتابیس همه را دارد). `run_tests`/`run_build` دقیقاً به همین دلیل check-محورند. اجرای build/test روی runner ایزوله یک قابلیت **جدا و پیاده‌سازی‌نشده** است.
- **Tests:** `src/tests/repo-context-visibility.test.ts` (۱۷ تست: ریپوی ۹۰۰ فایلی با یک درخواست tree، کش تفکیک‌شده per account، 404 → دلیل+راهنما، خوددرمانی شاخه، timeout، ریپوی خالی، پروژه بدون ریپو، متن پرامپت در دو حالت، `repo-status` سالم/شاخهٔ غلط/ریپوی مخفی/۴۰۴ کاربر دیگر، و ۳ تست jsdom برای بنر چت). `agent-github-contract` با ۴ تست جدید (مسیر سریع، فیلتر زیرمسیر، fallback روی truncated/شکل نامعتبر، پیام 404 با نام ریپو) به‌روز شد؛ تست قدیمیِ پیاده‌روی Contents حالا ۳ فراخوانی را انتظار دارد (۱ پروب tree + ۲ پوشه).
- **مستند:** `docs/TROUBLESHOOTING.md` (جدول بنر + علت‌ها)، `docs/API.md` (روت جدید)، `docs/ENVIRONMENT.md` و `.env.example` (`REPO_BRIEF_TTL_MS`), `TESTING.md` (سناریوی دستی).

## 2026-09-15 — آینهٔ محلی فقط‌خواندنی ریپازیتوری (Read-only repo mirror)

- **درخواست کاربر (فارسی):** «یک کلون محلی از پروژه باشد تا ایجنت‌ها عمیق‌تر/سریع‌تر کد را ببینند» — ولی **بدون** تبدیل CodeVia به میزبان اجرای کد. تصمیم صریح کاربر: آینهٔ **فقط‌خواندنی**، نه runner.
- **ماژول جدید `src/github/repo-mirror.ts` → `RepoMirrorService`:**
  - مسیر: `<REPO_MIRROR_DIR>/<acct-<sha1-16>>/<owner>/<name>.git` — **به تفکیک حساب کاربری فعال**؛ بدون توکن → اسکوپ `public`. یعنی «خواندنی بودن» و محتوای ریپوی private هرگز بین حساب‌ها به اشتراک نمی‌رود (همان فرهنگ S01).
  - clone با `git clone --mirror` و به‌روزرسانی با `git remote update --prune`؛ **bare** است، پس هیچ checkout/working tree وجود ندارد.
  - اعتبارنامه فقط با `GIT_CONFIG_COUNT/KEY_0=http.extraHeader/VALUE_0="AUTHORIZATION: bearer <token>"` در **env همان فرمان**: نه در argv (که در `ps` دیده می‌شود)، نه در کانفیگ ماندگار مخزن، نه در لاگ (`redact()` همهٔ پیام‌ها را می‌شوید). `GIT_TERMINAL_PROMPT=0` + `GIT_CONFIG_NOSYSTEM=1`.
  - فقط **plumbing**: `ls-tree -r --name-only` (کل درخت)، `cat-file -s`/`cat-file blob` (اندازه و محتوای یک فایل)، `rev-parse`/`symbolic-ref`/`for-each-ref` (SHA و شاخه‌ها)، `grep -I -n -F` (جست‌وجوی متن — چیزی که REST API ندارد). همه با `execFile` و argv ثابت (بدون shell)، با تایم‌اوت و سقف خروجی.
  - ایمنی/منابع: `safeName/safeRef/safePath` (رد `..`، `/` مطلق، ref شروع‌شده با `-`)، سقف `REPO_MIRROR_MAX_MB` (با `count-objects` و در صورت تخلف حذف آینه)، `REPO_MIRROR_MAX_FILES`، `REPO_MIRROR_MAX_READ_BYTES`، `REPO_MIRROR_MAX_SEARCH_HITS`، semaphore(2) برای clone/fetch هم‌زمان، TTL تازگی (`REPO_MIRROR_REFRESH_MS`) و dedupe درون‌پروازی per-path.
  - API: `detectGit`, `sync`, `status` (فقط دیسک، بدون شبکه)، `describe`, `listFiles`, `readFile`, `headSha`, `search`, `remove`, `invalidate`, `diskUsageMb` + `getRepoMirror()`/`setRepoMirrorForTest()`.
  - **هیچ کدی از مخزن اجرا نمی‌شود** — نه checkout، نه `npm install`، نه build/test. نوشتن همچنان Git Data API است و تست همچنان Check Runهای GitHub Actions.
- **اتصال به پلتفرم:**
  - `src/app/container.ts`: `get repoMirror()` (lazy تا تست بتواند inject کند) و `githubTokenForProject()`؛ `resolveGitHubTokenForProject()` جدید در `src/github/registry.ts` **دقیقاً همان تقدم** `resolveGitHubForProject` را دارد (توکن خودِ کاربر فعال → اتصال پروژه → `GITHUB_TOKEN` فقط وقتی server GitHub فعال است) و در نبود توکن `undefined` می‌دهد = clone ناشناس برای ریپوی public.
  - `src/agents/repo-brief.ts`: آینه **اول** امتحان می‌شود (`sync` داخل همان race با بودجهٔ چت) و `readOnce` یک `MirrorAccess` می‌گیرد؛ `via: "mirror" | "api"` و بلوک `mirror` (ready/sizeMb/ageMs/blocker/error) به brief و به `repoContextFor()` اضافه شد. هر مشکل آینه → همان مسیر API؛ ref نامعتبر هرگز بی‌صدا به HEAD تنزل نمی‌کند.
  - `GET /projects/:id/repo-status`: بلوک `mirror` (enabled/gitAvailable/gitVersion/root/ready/sizeMb/ageMs/refreshInMs/blocker/error) + دو روت جدید `POST /projects/:id/repo-mirror/refresh` (fetch اجباری + `invalidateRepoBrief`) و `DELETE /projects/:id/repo-mirror` (حذف نسخهٔ محلی؛ گیت‌هاب دست‌نخورده).
  - ابزار `search` (`src/tools/core-tools.ts`): لیست مسیرها اول از `ls-tree` آینه (وگرنه API) و **بخش جدید: `[match] path:line — text` از `git grep`**؛ `data.filesVia`/`data.matchesVia` می‌گوید شاهد از کجا آمده. `ToolContext` فیلدهای `mirror`/`mirrorScope`/`mirrorToken` گرفت و `AgentRunner` آن‌ها را با `req.requestUserId ?? project.ownerId` پر می‌کند.
- **زیرساخت:** `Dockerfile` در stage runtime حالا `git ca-certificates` نصب می‌کند (`node:22-slim` گیت نداشت → بدون آن آینه همیشه «git not installed» می‌گفت) و `REPO_MIRROR_DIR=/app/data/mirrors` روی همان volume مانا تنظیم شد؛ `docker-compose.yml` هم `REPO_MIRROR_*` را پاس می‌دهد.
- **UI:** `mirrorBitHtml()` در `client/app/140-conversations-memory-github.js` یک خط صادقانه به بنر Repository context اضافه می‌کند: «📚 local read-only mirror ready · 38 MB · synced 12s ago — this evidence was read from disk» یا «unavailable (git is not installed / too-large / clone-failed) — evidence is read through the GitHub API». باندل با `node scripts/build-app.mjs` بازسازی و `from`/`to`های `client/app/manifest.json` دوباره محاسبه شد (6751 خط).
- **Tests:** `src/tests/repo-mirror.test.ts` (۱۰ تست با **fixture گیت واقعی**: clone/ls-tree/readFile/grep، جداسازی شاخه‌ها، refetch نکردن وقتی تازه است + `force`، مسیرهای جدا per-scope، **اعتبارنامه هرگز در argv یا کانفیگ مخزن نمی‌ماند**، رد ورودی ناامن، غیرفعال، بیش‌ازحد بزرگ، شکست clone، نگه‌داشتن نسخهٔ قبلی وقتی refresh شکست می‌خورد، remove).
- **Env جدید:** `REPO_MIRROR_ENABLED` (true)، `REPO_MIRROR_DIR`، `REPO_MIRROR_REFRESH_MS` (300000)، `REPO_MIRROR_TIMEOUT_MS` (120000)، `REPO_MIRROR_READ_TIMEOUT_MS` (20000)، `REPO_MIRROR_MAX_FILES` (20000)، `REPO_MIRROR_MAX_READ_BYTES` (512KB)، `REPO_MIRROR_MAX_MB` (2000)، `REPO_MIRROR_MAX_SEARCH_HITS` (60)، `REPO_MIRROR_URL_TEMPLATE`، `REPO_MIRROR_GIT_PATH`.
- **مستند:** `docs/AGENT_EXECUTION.md` (ردیف «clone روی سرور» در جدول + بخش جدید «آینهٔ محلی فقط‌خواندنی — چه چیزی هست و چه چیزی نیست»)، `docs/TROUBLESHOOTING.md` (۳ ردیف بنر + بخش mirror با curl‌های کاربردی)، `docs/ENVIRONMENT.md` و `.env.example` (جدول/کامنت `REPO_MIRROR_*`)، `README.md`.
